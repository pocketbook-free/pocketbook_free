...
import ua.pocketbook.fb2viewer.zlibrary.text.view.style.ZLTextStyleDecoration;
...
/*	private final void drawString(int x, int y, char[] str, int offset, int length, ZLTextWord.Mark mark, int shift) {
		drawString( x, y, str, offset, length, mark, shift, true) ;
	}*/
	
	public final void drowRect(Rect r){
		final ZLPaintContext context = Context;
		context.setFillColor(getSelectedBackgroundColor());
		context.fillRectangle(r.left, r.top, r.right, r.bottom);
		Log.i("SelectedBackgroundColor", "x="+r.left);
	}

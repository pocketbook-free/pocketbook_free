...
package ua.pocketbook.fb2viewer.zlibrary.ui.android.view;
...
import ua.pocketbook.fb2viewer.zlibrary.core.image.ZLImageData;
...
import ua.pocketbook.fb2viewer.zlibrary.ui.android.image.ZLAndroidImageData;
...
public final class ZLAndroidPaintContext extends ZLPaintContext {
	...
	private ZLAndroidPaintContext() {
		...
		mySelectPaint.setColor(Color.rgb(255, 127, 0));
		mySelectPaint.setStyle(Paint.Style.FILL);
	}
	...
	public void drawString(int x, int y, char[] string, int offset, int length) {
		...
		//if(ReaderActivity.instance.readerPreferences.isPanelDictionarVisible()){
		//final FBReader fbreader = (FBReader)FBReader.Instance(); 
		//Rect bounds = new Rect();
		//myTextPaint.getTextBounds(string, offset, length,  bounds);
		
		
		//bounds.left = x;
		//bounds.right = x + getStringWidth(string, offset, length);
		//bounds.top = y - getStringHeight() ;
		//bounds.bottom = y +getStringHeightInternal() -getStringHeight() ;
		
		//char[] dest = new char[length];
		//System.arraycopy(string, offset, dest, 0, length);
		//fbreader.BookTextView.textMaping.add(bounds, dest );
		
		//Log.i("Text "+new String(dest) , ""+bounds.top +" "+bounds.bottom+" "+bounds.left+" "+bounds.right);
		
		//Rect r = ((FBReader)FBReader.Instance()).BookTextView.textMaping.getRect();
		
	//	if(((FBReader)FBReader.Instance()).BookTextView.textMaping.isSelect(x,y)){
		
	//		myCanvas.drawRect(((FBReader)FBReader.Instance()).BookTextView.textMaping.getSelectRect(x,y), mySelectPaint);
	//	}

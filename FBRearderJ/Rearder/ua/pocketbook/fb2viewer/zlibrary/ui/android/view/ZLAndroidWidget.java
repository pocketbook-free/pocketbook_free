...
import ua.pocketbook.fb2viewer.zlibrary.core.view.ZLView;
import ua.pocketbook.fb2viewer.zlibrary.ui.android.util.ZLAndroidKeyUtil;
import ua.pocketbook.reader.R;
import ua.pocketbook.reader.ReaderActivity;

public class ZLAndroidWidget extends View {
	private final Paint myPaint = new Paint();
	public Bitmap myMainBitmap;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		try{
			if(!(ReaderActivity.readerPreferences.isPanelDictionarVisible() && findViewById(R.id.ll_dic_enterwords).getVisibility() == View.VISIBLE))
				((FBReader)FBReader.Instance()).BookTextView.textMaping.clear();
	...
	/*	try{				
			...
			int scrollsize = computeVerticalScrollExtent();
			int height = computeVerticalScrollRange();			
			int pageCount  = height/scrollsize;
			int curentPage = pageCount*(offset + scrollsize/2)/height;
			
			Log.i("offset", ""+offset);
			Log.i("scrollsize", ""+scrollsize);
			Log.i("height", ""+height);
			
			ReaderActivity.instance.setPageNumber(curentPage, pageCount);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_UP:
            	...
            	if(Math.abs(lastDownX - x) > 60)
            		view.scrollEvent(x, y, (int) -velocityTracker.getXVelocity(), (int) -velocityTracker.getYVelocity());

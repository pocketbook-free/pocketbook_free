...
import ua.pocketbook.fb2viewer.fbreader.formats.xhtml.XHTMLReader;
import ua.pocketbook.fb2viewer.zlibrary.core.html.*;
import ua.pocketbook.fb2viewer.zlibrary.core.util.ZLArrayUtils;
import ua.pocketbook.fb2viewer.zlibrary.core.xml.ZLXMLProcessor;
import ua.pocketbook.fb2viewer.zlibrary.text.model.ZLTextParagraph;

public class ChmHtmlReader extends BookReader implements ZLHtmlReader {
	...
	public ChmHtmlReader(BookModel model) throws UnsupportedEncodingException {
		super(model);
		try {				
			myAttributeDecoder = createDecoder();
			setByteDecoder(myAttributeDecoder);
	...
	protected final CharsetDecoder createDecoder() {
		...
		/*try {
			String e = MiscUtil.getEncodingByHtmlFile(new URL("file://"+Model.Book.File.getPath()));
			if (e!=null) encoding = e;
		} catch (MalformedURLException e) {	}
		
		Model.Book.setEncoding(encoding);
		Log.i("Encoding************", ""+Model.Book.getEncoding());*/
	...
	public boolean readBook(InputStream inputStream) throws IOException {
		return ZLHtmlProcessor.read(this, inputStream);
	}

...
package ua.pocketbook.fb2viewer.fbreader.library;
...
import ua.pocketbook.fb2viewer.fbreader.formats.*;
import ua.pocketbook.fb2viewer.zlibrary.core.filesystem.*;
import ua.pocketbook.fb2viewer.zlibrary.core.resources.ZLResource;
import ua.pocketbook.fb2viewer.zlibrary.core.util.ZLMiscUtil;
...
/*	public ZLTextPosition getStoredPosition(ZLTextModel model) {
		
		//TODO
		
		Log.i("File.getPath()",""+File.getPath());
		BookStruct bookStruct = ReaderActivity.bookContentResolver.getIdBookByFileName(File.getPath());	
		ZLTextWordCursor cursor = new ZLTextWordCursor();
		cursor.setCursor(model, bookStruct.getReaderposition());
		return cursor;
	...
	/*	if (myId != -1) {
		...
		Log.i("storePosition", position.toString());
		bookStruct.setReaderposition(position.toString());
		ReaderActivity.bookContentResolver.updateBook(bookStruct);

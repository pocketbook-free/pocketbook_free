...
abstract class ZLTreeAdapter extends BaseAdapter implements AdapterView.OnItemClickListener, View.OnCreateContextMenuListener {
	...
	protected final void setIcon(ImageView imageView, ZLTree tree) {
		if (tree.hasChildren()) {
			...
			if(((TOCTree)tree).myReference != null && ((TOCTree)tree).myReference.isNoteStruct ){
				if(((TOCTree)tree).myReference.isNotes)
					imageView.setImageResource(R.drawable.main_menu_note);
				else
					imageView.setImageResource(R.drawable.main_menu_bookmark);
			}else{

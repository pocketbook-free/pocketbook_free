...
inline int EmbedStream::getChar() {
  if (limited && !length) {
    return EOF;
  }
  --length;
...
if (colorXform && cc != 0 && cc != 3) continue;
...
*/
	...
	// convert YCbCrK to CMYK (K is passed through unchanged)
	  ...
	  for (y2 = 0; y2 < mcuHeight; ++y2) {
	    ...
	    p1 = &frameBuf[1][(y1+y2) * bufWidth + x1];
	    p2 = &frameBuf[2][(y1+y2) * bufWidth + x1];
	    p3 = &frameBuf[3][(y1+y2) * bufWidth + x1];
	    for (x2 = 0; x2 < mcuWidth; ++x2) {
              *(p1++) = *(p2++) = (*(p0++) * (256 - *(p3++))) >> 8;
/*
	      pY = *p0;
...
void DCTStream::transformDataUnit(Gushort *quantTable,
  ...
  for (i = 0; i < 64; i += 8) {
    ...
    if (p[1] < VMIN && p[2] < VMIN && p[3] < VMIN &&
	p[4] < VMIN && p[5] < VMIN && p[6] < VMIN && p[7] < VMIN) {
  ...
  for (i = 0; i < 8; ++i) {
    ...
    // check for all-zero AC coefficients
    if (p[1*8] < VMIN && p[2*8] < VMIN && p[3*8] < VMIN &&
	p[4*8] < VMIN && p[5*8] < VMIN && p[6*8] < VMIN && p[7*8] < VMIN) {

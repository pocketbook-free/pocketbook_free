...
static FixedPoint MAXFP = (FixedPoint)32767.0;
static FixedPoint FP0 = (FixedPoint)0;
static FixedPoint FP1 = (FixedPoint)1;

static inline GfxColorComp clip01(GfxColorComp x) {
  return (x < 0) ? 0 : (x > gfxColorComp1) ? gfxColorComp1 : x;
}

static inline FixedPoint clip01(FixedPoint x) {
  return (x < FP0) ? FP0 : (x > FP1) ? FP1 : x;
}
...
void Matrix::transform(FixedPoint x, FixedPoint y, FixedPoint *tx, FixedPoint *ty)
{
  FixedPoint temp_x, temp_y;
...
void GfxColorSpace::getDefaultRanges(FixedPoint *decodeLow, FixedPoint *decodeRange,
				     int maxImgPixel) {
  int i;
...
GfxColorSpace *GfxCalGrayColorSpace::parse(Array *arr) {
  ...
  if (obj1.dictLookup("WhitePoint", &obj2)->isArray() &&
    ...
    cs->whiteX = obj3.getFP();
    obj3.free();
    obj2.arrayGet(1, &obj3);
    cs->whiteY = obj3.getFP();
    obj3.free();
    obj2.arrayGet(2, &obj3);
    cs->whiteZ = obj3.getFP();
    obj3.free();
  }
  obj2.free();
  if (obj1.dictLookup("BlackPoint", &obj2)->isArray() &&
      obj2.arrayGetLength() == 3) {
    obj2.arrayGet(0, &obj3);
    cs->blackX = obj3.getFP();
    obj3.free();
    obj2.arrayGet(1, &obj3);
    cs->blackY = obj3.getFP();
    obj3.free();
    obj2.arrayGet(2, &obj3);
    cs->blackZ = obj3.getFP();
    obj3.free();
  }
  obj2.free();
...
void GfxDeviceRGBColorSpace::getGray(GfxColor *color, GfxGray *gray) {
  *gray = clip01((GfxColorComp)
	((color->c[0] * 77 + color->c[1] * 151 + color->c[2] * 28) >> 8));
/*
		(0.3 * color->c[0] +
		 0.59 * color->c[1] +
		 0.11 * color->c[2] + 0.5));
...
GfxColorSpace *GfxCalRGBColorSpace::parse(Array *arr) {
  ...
  if (obj1.dictLookup("Gamma", &obj2)->isArray() &&
      obj2.arrayGetLength() == 3) {
    obj2.arrayGet(0, &obj3);
    cs->gammaR = obj3.getFP();
    obj3.free();
    obj2.arrayGet(1, &obj3);
    cs->gammaG = obj3.getFP();
    obj3.free();
    obj2.arrayGet(2, &obj3);
    cs->gammaB = obj3.getFP();
    obj3.free();
  }
  obj2.free();
...
inline void GfxCalRGBColorSpace::getGray(GfxColor *color, GfxGray *gray) {
  *gray = clip01((GfxColorComp)
	((color->c[0] * 77 + color->c[1] * 151 + color->c[2] * 28) >> 8));
/*
		(0.299 * color->c[0] +
...
void GfxDeviceCMYKColorSpace::getGray(GfxColor *color, GfxGray *gray) {
  *gray = clip01((GfxColorComp)
	((gfxColorComp1 * 256 - color->c[3] * 256 - color->c[0] * 77 + color->c[1] * 151 + color->c[2] * 28) >> 8));
/*
		(gfxColorComp1 - color->c[3]
				- 0.3  * color->c[0]
				- 0.59 * color->c[1]
				- 0.11 * color->c[2] + 0.5));
*/
...
/*
  FixedPoint c, m, y, k, c1, m1, y1, k1, r, g, b, x;
  ...
  g += 0.2119 * x;
  b += 0.2235 * x;
  rgb->r = clip01(fpToCol(r));
  rgb->g = clip01(fpToCol(g));
  rgb->b = clip01(fpToCol(b));
*/
}
...
GfxColorSpace *GfxLabColorSpace::parse(Array *arr) {
  ...
  cs->kr = (FixedPoint)1 / (xyzrgb[0][0] * cs->whiteX +
		xyzrgb[0][1] * cs->whiteY +
		xyzrgb[0][2] * cs->whiteZ);
  cs->kg = (FixedPoint)1 / (xyzrgb[1][0] * cs->whiteX +
		xyzrgb[1][1] * cs->whiteY +
		xyzrgb[1][2] * cs->whiteZ);
  cs->kb = (FixedPoint)1 / (xyzrgb[2][0] * cs->whiteX +
...
void GfxLabColorSpace::getRGB(GfxColor *color, GfxRGB *rgb) {
  ...
  if (t2 >= (FixedPoint)(6.0 / 29.0)) {
    X = t2 * t2 * t2;
  } else {
    X = (FixedPoint)(108.0 / 841.0) * (t2 - (FixedPoint)(4.0 / 29.0));
  }
  X *= whiteX;
  if (t1 >= (6.0 / 29.0)) {
    Y = t1 * t1 * t1;
  } else {
    Y = (FixedPoint)(108.0 / 841.0) * (t1 - (FixedPoint)(4.0 / 29.0));
  }
  Y *= whiteY;
  ...
  if (t2 >= (FixedPoint)(6.0 / 29.0)) {
    ...
    Z = (FixedPoint)(108.0 / 841.0) * (t2 - (FixedPoint)(4.0 / 29.0));
  }
  Z *= whiteZ;
  ...
  rgb->r = fpToCol(FixedPoint::pow(clip01(r * kr), (FixedPoint)0.5));
  rgb->g = fpToCol(FixedPoint::pow(clip01(g * kg), (FixedPoint)0.5));
  rgb->b = fpToCol(FixedPoint::pow(clip01(b * kb), (FixedPoint)0.5));
}
...
void GfxLabColorSpace::getDefaultColor(GfxColor *color) {
  color->c[0] = 0;
  if (aMin > 0) {
    color->c[1] = fpToCol(aMin);
  } else if (aMax < 0) {
    color->c[1] = fpToCol(aMax);
  } else {
    color->c[1] = 0;
  }
  if (bMin > 0) {
    color->c[2] = fpToCol(bMin);
  } else if (bMax < 0) {
    color->c[2] = fpToCol(bMax);
  } else {
    color->c[2] = 0;
  }
}

void GfxLabColorSpace::getDefaultRanges(FixedPoint *decodeLow, FixedPoint *decodeRange,
					int maxImgPixel) {
  decodeLow[0] = 0;
  decodeRange[0] = 100;
  decodeLow[1] = aMin;
...
GfxColorSpace *GfxICCBasedColorSpace::parse(Array *arr) {
  ...
  if (dict->lookup("Range", &obj2)->isArray() &&
    ...
    for (i = 0; i < nCompsA; ++i) {
      ...
      cs->rangeMin[i] = obj3.getFP();
      obj3.free();
      obj2.arrayGet(2*i+1, &obj3);
      cs->rangeMax[i] = obj3.getFP();
      obj3.free();
    }
  }
  obj2.free();
...
void GfxICCBasedColorSpace::getDefaultColor(GfxColor *color) {
  ...
  for (i = 0; i < nComps; ++i) {
    if (rangeMin[i] > 0) {
      color->c[i] = fpToCol(rangeMin[i]);
    } else if (rangeMax[i] < 0) {
      color->c[i] = fpToCol(rangeMax[i]);
    } else {
      color->c[i] = 0;
    }
  }
...
void GfxICCBasedColorSpace::getDefaultRanges(FixedPoint *decodeLow,
					     FixedPoint *decodeRange,
					     int maxImgPixel) {
...
GfxColor *GfxIndexedColorSpace::mapColorToBase(GfxColor *color,
					       GfxColor *baseColor) {
  Guchar *p;
  FixedPoint low[gfxColorMaxComps], range[gfxColorMaxComps];
  int n, i;
  FixedPoint V255 = (FixedPoint) 255.0;

  n = base->getNComps();
  base->getDefaultRanges(low, range, indexHigh);
  p = &lookup[(int)(colToDbl(color->c[0]) + 0.5) * n];
  for (i = 0; i < n; ++i) {
    baseColor->c[i] = fpToCol(low[i] + ((FixedPoint)p[i] / V255) * range[i]);
  }
  return baseColor;
}
...
void GfxIndexedColorSpace::getDefaultRanges(FixedPoint *decodeLow,
					    FixedPoint *decodeRange,
					    int maxImgPixel) {
  decodeLow[0] = 0;
...
void GfxDeviceNColorSpace::getGray(GfxColor *color, GfxGray *gray) {
  FixedPoint x[gfxColorMaxComps], c[gfxColorMaxComps];
  GfxColor color2;
  int i;
...
GfxTilingPattern *GfxTilingPattern::parse(Object *patObj) {
  ...
  FixedPoint bboxA[4], matrixA[6];
  FixedPoint xStepA, yStepA;
  Object resDictA;
  Object obj1, obj2;
  int i;
...
GfxShadingPattern::GfxShadingPattern(GfxShading *shadingA, FixedPoint *matrixA):
  GfxPattern(2)
{
  int i;
...
GBool GfxShading::init(Dict *dict) {
  ...
  if (dict->lookup("Background", &obj1)->isArray()) {
	...
	background.c[i] = fpToCol(obj1.arrayGet(i, &obj2)->getFP());
	obj2.free();
      }
    } else {
  ...
  if (dict->lookup("BBox", &obj1)->isArray()) {
    if (obj1.arrayGetLength() == 4) {
      hasBBox = gTrue;
      xMin = obj1.arrayGet(0, &obj2)->getFP();
      obj2.free();
      yMin = obj1.arrayGet(1, &obj2)->getFP();
      obj2.free();
      xMax = obj1.arrayGet(2, &obj2)->getFP();
      obj2.free();
      yMax = obj1.arrayGet(3, &obj2)->getFP();
      obj2.free();
    } else {
...
GfxFunctionShading::GfxFunctionShading(FixedPoint x0A, FixedPoint y0A,
...
GfxFunctionShading *GfxFunctionShading::parse(Dict *dict) {
  ...
  if (dict->lookup("Domain", &obj1)->isArray() &&
      obj1.arrayGetLength() == 4) {
    x0A = obj1.arrayGet(0, &obj2)->getFP();
    obj2.free();
    x1A = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
    y0A = obj1.arrayGet(2, &obj2)->getFP();
    obj2.free();
    y1A = obj1.arrayGet(3, &obj2)->getFP();
    obj2.free();
  }
  obj1.free();
  ...
  if (dict->lookup("Matrix", &obj1)->isArray() &&
      obj1.arrayGetLength() == 6) {
    matrixA[0] = obj1.arrayGet(0, &obj2)->getFP();
    obj2.free();
    matrixA[1] = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
    matrixA[2] = obj1.arrayGet(2, &obj2)->getFP();
    obj2.free();
    matrixA[3] = obj1.arrayGet(3, &obj2)->getFP();
    obj2.free();
    matrixA[4] = obj1.arrayGet(4, &obj2)->getFP();
    obj2.free();
    matrixA[5] = obj1.arrayGet(5, &obj2)->getFP();
    obj2.free();
  }
  obj1.free();
...
void GfxFunctionShading::getColor(FixedPoint x, FixedPoint y, GfxColor *color) {
  FixedPoint in[2], out[gfxColorMaxComps];
  int i;
...
GfxAxialShading::GfxAxialShading(FixedPoint x0A, FixedPoint y0A,
				 FixedPoint x1A, FixedPoint y1A,
...
GfxAxialShading *GfxAxialShading::parse(Dict *dict) {
  ...
  if (dict->lookup("Coords", &obj1)->isArray() &&
    ...
    x0A = obj1.arrayGet(0, &obj2)->getFP();
    obj2.free();
    y0A = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
    x1A = obj1.arrayGet(2, &obj2)->getFP();
    obj2.free();
    y1A = obj1.arrayGet(3, &obj2)->getFP();
    obj2.free();
  } else {
  ...
  if (dict->lookup("Domain", &obj1)->isArray() &&
    ...
    t0A = obj1.arrayGet(0, &obj2)->getFP();
    obj2.free();
    t1A = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
  }
  obj1.free();
...
void GfxAxialShading::getColor(FixedPoint t, GfxColor *color) {
  FixedPoint out[gfxColorMaxComps];
  int i;
...
GfxRadialShading::GfxRadialShading(FixedPoint x0A, FixedPoint y0A, FixedPoint r0A,
...
GfxRadialShading *GfxRadialShading::parse(Dict *dict) {
  ...
  FixedPoint x0A, y0A, r0A, x1A, y1A, r1A;
  FixedPoint t0A, t1A;
  Function *funcsA[gfxColorMaxComps];
  int nFuncsA;
  GBool extend0A, extend1A;
  ...
  if (dict->lookup("Coords", &obj1)->isArray() &&
    ...
    y0A = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
    r0A = obj1.arrayGet(2, &obj2)->getFP();
    obj2.free();
    x1A = obj1.arrayGet(3, &obj2)->getFP();
    obj2.free();
    y1A = obj1.arrayGet(4, &obj2)->getFP();
    obj2.free();
    r1A = obj1.arrayGet(5, &obj2)->getFP();
    obj2.free();
  } else {
    error(-1, "Missing or invalid Coords in shading dictionary");
    goto err1;
...
void GfxRadialShading::getColor(FixedPoint t, GfxColor *color) {
  FixedPoint out[gfxColorMaxComps];
  int i;
...
GfxGouraudTriangleShading *GfxGouraudTriangleShading::parse(int typeA,
  ...
  FixedPoint cMin[gfxColorMaxComps], cMax[gfxColorMaxComps];
  FixedPoint xMul, yMul;
  FixedPoint cMul[gfxColorMaxComps];
  ...
  if (dict->lookup("Decode", &obj1)->isArray() &&
    ...
    xMin = obj1.arrayGet(0, &obj2)->getFP();
    obj2.free();
    xMax = obj1.arrayGet(1, &obj2)->getFP();
    obj2.free();
    xMul = (xMax - xMin) / (pow(2.0, coordBits) - 1);
    yMin = obj1.arrayGet(2, &obj2)->getFP();
    obj2.free();
    yMax = obj1.arrayGet(3, &obj2)->getFP();
    obj2.free();
    yMul = (yMax - yMin) / (pow(2.0, coordBits) - 1);
    for (i = 0; 5 + 2*i < obj1.arrayGetLength() && i < gfxColorMaxComps; ++i) {
      cMin[i] = obj1.arrayGet(4 + 2*i, &obj2)->getFP();
      obj2.free();
      cMax[i] = obj1.arrayGet(5 + 2*i, &obj2)->getFP();
      obj2.free();
      cMul[i] = (cMax[i] - cMin[i]) / (FixedPoint)((1 << compBits) - 1);
    }
    nComps = i;
  } else {
  ...
  while (1) {
    ...
    verticesA[nVerticesA].x = xMin + xMul * (FixedPoint)(int)x;
    verticesA[nVerticesA].y = yMin + yMul * (FixedPoint)(int)y;
    for (i = 0; i < nComps; ++i) {
      verticesA[nVerticesA].color.c[i] =
	  fpToCol(cMin[i] + cMul[i] * (FixedPoint)(int)c[i]);
    }
    ++nVerticesA;
    bitBuf->flushBits();
    if (typeA == 4) {
...
void GfxGouraudTriangleShading::getTriangle(
				    int i,
				    FixedPoint *x0, FixedPoint *y0, GfxColor *color0,
...
GfxPatchMeshShading *GfxPatchMeshShading::parse(int typeA, Dict *dict,
  ...
  Object obj1, obj2;
  int i, j;
  FixedPoint V4 = (FixedPoint)4;
  FixedPoint V6 = (FixedPoint)6;
  FixedPoint V2 = (FixedPoint)2;
  FixedPoint V3 = (FixedPoint)3;
  FixedPoint V9 = (FixedPoint)9;
  ...
  while (1) {
	...
	break;
      ...
      x[i] = xMin + xMul * (FixedPoint)(int)xi;
      y[i] = yMin + yMul * (FixedPoint)(int)yi;
    }
    if (i < nPts) {
      break;
    }
	...
	c[i][j] = fpToCol(cMin[j] + cMul[j] * (FixedPoint)(int)ci[j]);
      }
      if (j < nComps) {
	break;
      }
...
GfxImageColorMap::GfxImageColorMap(int bitsA, Object *decode,
  ...
  Function *sepFunc;
  Object obj;
  FixedPoint x[gfxColorMaxComps];
  FixedPoint y[gfxColorMaxComps];
  int i, j, k, byte;
  FixedPoint mapped;
  FixedPoint V05 = (FixedPoint) 0.5;
  FixedPoint V255 = (FixedPoint) 255;
  ...
  if (decode->isNull()) {
	...
	goto err2;
      ...
      decodeLow[i] = obj.getFP();
      obj.free();
      decode->arrayGet(2*i+1, &obj);
      if (!obj.isNum()) {
	goto err2;
      }
      decodeRange[i] = obj.getFP() - decodeLow[i];
      obj.free();
    }
  } else {
    goto err1;
  ...
  if (colorSpace->getMode() == csIndexed) {
	...
	j = (int)(decodeLow[0] + ((FixedPoint)i / (FixedPoint)maxPixel) * decodeRange[0] + V05);
	if (j < 0) {
	  j = 0;
	} else if (j > indexHigh) {
	  j = indexHigh;
	...
	mapped = x[k] + ((FixedPoint)lookup2[j*nComps2 + k] / V255) * y[k];
	lookup[k][i] = fpToCol(mapped);
	byte_lookup[i * nComps2 + k] = (Guchar)(int) (mapped * V255);
      }
    }
    ...
    for (k = 0; k < nComps2; ++k) {
      lookup[k] = (GfxColorComp *)gmallocn(maxPixel + 1,
					   sizeof(GfxColorComp));
      for (i = 0; i <= maxPixel; ++i) {
	x[0] = decodeLow[0] + ((FixedPoint)i / (FixedPoint)maxPixel) * decodeRange[0];
	sepFunc->transform(x, y);
	lookup[k][i] = fpToCol(y[k]);
	byte_lookup[i*nComps2 + k] = (Guchar)(int) (y[k] * V255);
      }
    }
  } else {
    ...
    for (k = 0; k < nComps; ++k) {
      lookup[k] = (GfxColorComp *)gmallocn(maxPixel + 1,
					   sizeof(GfxColorComp));
      for (i = 0; i <= maxPixel; ++i) {
	mapped = decodeLow[k] + ((FixedPoint)i / (FixedPoint)maxPixel) * decodeRange[k];
	lookup[k][i] = fpToCol(mapped);
	byte = (int) (mapped * V255 + V05);
	if (byte < 0)  
	  byte = 0;  
	else if (byte > 255)  
	  byte = 255;  
...
void GfxImageColorMap::getColor(Guchar *x, GfxColor *color) {
  ...
  maxPixel = (1 << bits) - 1;
  for (i = 0; i < nComps; ++i) {
    color->c[i] = fpToCol(decodeLow[i] + ((FixedPoint)x[i] / (FixedPoint)maxPixel) * decodeRange[i]);
  }
}
...
GfxSubpath::GfxSubpath(FixedPoint x1, FixedPoint y1) {
  size = 16;
  x = (FixedPoint *)gmallocn(size, sizeof(FixedPoint));
  y = (FixedPoint *)gmallocn(size, sizeof(FixedPoint));
...
GfxSubpath::GfxSubpath(GfxSubpath *subpath) {
  ...
  y = (FixedPoint *)gmallocn(size, sizeof(FixedPoint));
  curve = (GBool *)gmallocn(size, sizeof(GBool));
  memcpy(x, subpath->x, n * sizeof(FixedPoint));
  memcpy(y, subpath->y, n * sizeof(FixedPoint));
  memcpy(curve, subpath->curve, n * sizeof(GBool));
  closed = subpath->closed;
}

void GfxSubpath::lineTo(FixedPoint x1, FixedPoint y1) {
  if (n >= size) {
    size += 16;
    x = (FixedPoint *)greallocn(x, size, sizeof(FixedPoint));
    y = (FixedPoint *)greallocn(y, size, sizeof(FixedPoint));
...
void GfxSubpath::curveTo(FixedPoint x1, FixedPoint y1, FixedPoint x2, FixedPoint y2,
...
void GfxSubpath::offset(FixedPoint dx, FixedPoint dy) {
  int i;
...
// Used for copy().
GfxPath::GfxPath(GBool justMoved1, FixedPoint firstX1, FixedPoint firstY1,
...
void GfxPath::moveTo(FixedPoint x, FixedPoint y) {
  justMoved = gTrue;
  firstX = x;
  firstY = y;
}

void GfxPath::lineTo(FixedPoint x, FixedPoint y) {
  if (justMoved) {
    if (n >= size) {
      size += 16;
      subpaths = (GfxSubpath **)
...
void GfxPath::curveTo(FixedPoint x1, FixedPoint y1, FixedPoint x2, FixedPoint y2,
	     FixedPoint x3, FixedPoint y3) {
  if (justMoved) {
    if (n >= size) {
      size += 16;
...
void GfxPath::offset(FixedPoint dx, FixedPoint dy) {
  int i;
...
GfxState::GfxState(FixedPoint hDPIA, FixedPoint vDPIA, PDFRectangle *pageBox,
...
GfxState::GfxState(GfxState *state) {
  ...
  if (lineDashLength > 0) {
    lineDash = (FixedPoint *)gmallocn(lineDashLength, sizeof(FixedPoint));
    memcpy(lineDash, state->lineDash, lineDashLength * sizeof(FixedPoint));
  }
  if (font)
    font->incRefCnt();
...
void GfxState::getUserClipBBox(FixedPoint *xMin, FixedPoint *yMin,
			       FixedPoint *xMax, FixedPoint *yMax) {
  FixedPoint ictm[6];
  FixedPoint xMin1, yMin1, xMax1, yMax1, det, tx, ty;

  // invert the CTM
  det = (FixedPoint)1 / (ctm[0] * ctm[3] - ctm[1] * ctm[2]);
  ictm[0] = ctm[3] * det;
  ictm[1] = -ctm[1] * det;
  ictm[2] = -ctm[2] * det;
  ictm[3] = ctm[0] * det;
...
FixedPoint GfxState::transformWidth(FixedPoint w) {
  FixedPoint x, y;

  static FixedPoint  V05 = (FixedPoint)0.5;

  x = ctm[0] + ctm[2];
  y = ctm[1] + ctm[3];
  return w * sqrt(V05 * (x * x + y * y));
}

FixedPoint GfxState::getTransformedFontSize() {
...
void GfxState::getFontTransMat(FixedPoint *m11, FixedPoint *m12,
...
void GfxState::setCTM(FixedPoint a, FixedPoint b, FixedPoint c,
...
void GfxState::concatCTM(FixedPoint a, FixedPoint b, FixedPoint c,
			 FixedPoint d, FixedPoint e, FixedPoint f) {
  FixedPoint a1 = ctm[0];
  FixedPoint b1 = ctm[1];
  FixedPoint c1 = ctm[2];
...
void GfxState::shiftCTM(FixedPoint tx, FixedPoint ty) {
  ctm[4] += tx;
  ctm[5] += ty;
  clipXMin += tx;
  clipYMin += ty;
...
void GfxState::setFont(GfxFont *fontA, FixedPoint fontSizeA) {
  if (font)
    font->decRefCnt();
...
void GfxState::setLineDash(FixedPoint *dash, int length, FixedPoint start) {
  if (lineDash)
    gfree(lineDash);
  lineDash = dash;
...
void GfxState::clipToStrokePath() {
  FixedPoint xMin, yMin, xMax, yMax, x, y, t0, t1;
  GfxSubpath *subpath;
  int i, j;
  static FixedPoint V05 = (FixedPoint)0.5;
  ...
  t1 = fabs(ctm[2]);
  if (t0 > t1) {
    xMin -= V05 * lineWidth * t0;
    xMax += V05 * lineWidth * t0;
  } else {
    xMin -= V05 * lineWidth * t1;
    xMax += V05 * lineWidth * t1;
  }
  t0 = fabs(ctm[0]);
  t1 = fabs(ctm[3]);
  if (t0 > t1) {
    yMin -= V05 * lineWidth * t0;
    yMax += V05 * lineWidth * t0;
  } else {
    yMin -= V05 * lineWidth * t1;
    yMax += V05 * lineWidth * t1;
  }
...
void GfxState::textShift(FixedPoint tx, FixedPoint ty) {
  FixedPoint dx, dy;
...
void GfxState::shift(FixedPoint dx, FixedPoint dy) {
  curX += dx;
  curY += dy;
}

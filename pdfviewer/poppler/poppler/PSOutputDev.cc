...
struct PSSubstFont {
  ...
  FixedPoint mWidth;		// width of 'm' character
};
...
public:
  ...
  PSOutCustomColor(FixedPoint cA, FixedPoint mA,
		   FixedPoint yA, FixedPoint kA, GooString *nameA);
  ~PSOutCustomColor();
...
PSOutCustomColor::PSOutCustomColor(FixedPoint cA, FixedPoint mA,
				   FixedPoint yA, FixedPoint kA, GooString *nameA) {
  c = cA;
  m = mA;
  y = yA;
...
GBool DeviceNRecoder::fillBuf() {
  ...
  GfxColor color;
  FixedPoint x[gfxColorMaxComps], y[gfxColorMaxComps];
  int i;
...
void PSOutputDev::writeTrailer() {
  ...
  if (mode == psModeForm) {
	...
	writePSFmt("%%+ {0:.4g} {1:.4g} {2:.4g} {3:.4g} ({4:t})\n",
		   (double)cc->c, (double)cc->m, (double)cc->y, (double)cc->k, cc->name);
      }
    }
  }
}
...
void PSOutputDev::setupFont(GfxFont *font, Dict *parentResDict) {
  ...
  if (font->isCIDFont()) {
    ...
    writePSFmt("/F{0:d}_{1:d} /{2:t} {3:.4g} {4:.4g}\n",
	       font->getID()->num, font->getID()->gen, psName, (double)xs, (double)ys);
    for (i = 0; i < 256; i += 8) {
      writePS((char *)((i == 0) ? "[ " : "  "));
      for (j = 0; j < 8; ++j) {
	...
	// uses a FixedPoint-slash (//...) for "immediately evaluated names",
  ...
  if ((charProcs = ((Gfx8BitFont *)font)->getCharProcs())) {
	...
	if (t3Cacheable) {
	  buf = GooString::format("{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} setcachedevice\n",
				(double)t3WX, (double)t3WY, (double)t3LLX, (double)t3LLY, (double)t3URX, (double)t3URY);
	} else {
	  buf = GooString::format("{0:.4g} {1:.4g} setcharwidth\n", (double)t3WX, (double)t3WY);
	}
...
GBool PSOutputDev::checkPageSlice(Page *page, FixedPoint /*hDPI*/, FixedPoint /*vDPI*/,
...
void PSOutputDev::startPage(int pageNum, GfxState *state) {
  ...
  case psModePS:
	...
	(globalParams->getPSExpandSmaller() &&
	 (width < imgWidth2 && height < imgHeight2))) {
      xScale = (FixedPoint)imgWidth2 / (FixedPoint)width;
      yScale = (FixedPoint)imgHeight2 / (FixedPoint)height;
      if (yScale < xScale) {
	xScale = yScale;
      } else {
	yScale = xScale;
    ...
    if (tx0 >= 0 && ty0 >= 0) {
      ...
      if (clipLLX0 < clipURX0 && clipLLY0 < clipURY0) {
	tx += ((FixedPoint)imgWidth2 - xScale * (clipURX0 - clipLLX0)) / 2;
	ty += ((FixedPoint)imgHeight2 - yScale * (clipURY0 - clipLLY0)) / 2;
      } else {
	tx += ((FixedPoint)imgWidth2 - xScale * width) / 2;
	ty += ((FixedPoint)imgHeight2 - yScale * height) / 2;
      }
    }
    ...
    if (clipLLX0 < clipURX0 && clipLLY0 < clipURY0) {
      writePSFmt("{0:.4g} {1:.4g} {2:.4g} {3:.4g} re W\n",
		 (double)clipLLX0, (double)clipLLY0, (double)(clipURX0 - clipLLX0), (double)(clipURY0 - clipLLY0));
    } else {
      writePSFmt("{0:d} {1:d} {2:d} {3:d} re W\n", (double)x1, (double)y1, (double)(x2 - x1), (double)(y2 - y1));
    }
...
void PSOutputDev::updateCTM(GfxState *state, FixedPoint m11, FixedPoint m12,
...
void PSOutputDev::updateMiterLimit(GfxState *state) {
  writePSFmt("{0:.4g} M\n", (double)state->getMiterLimit());
}

void PSOutputDev::updateLineWidth(GfxState *state) {
  writePSFmt("{0:.4g} w\n", (double)state->getLineWidth());
}
...
void PSOutputDev::addProcessColor(FixedPoint c, FixedPoint m, FixedPoint y, FixedPoint k) {
  if (c > 0) {
    processColors |= psProcessCyan;
  }
  if (m > 0) {
...
void PSOutputDev::updateTextMat(GfxState *state) {
  ...
  if (fabs(mat[0] * mat[3] - mat[1] * mat[2]) < 0.00001) {
    ...
    writePSFmt("[{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g}] Tm\n",
	       (double)mat[0], (double)mat[1], (double)mat[2], (double)mat[3], (double)mat[4], (double)mat[5]);
  }
}

void PSOutputDev::updateCharSpace(GfxState *state) {
  writePSFmt("{0:.4g} Tc\n", (double)state->getCharSpace());
}
...
void PSOutputDev::updateRise(GfxState *state) {
  writePSFmt("{0:.4g} Ts\n", (double)state->getRise());
}

void PSOutputDev::updateWordSpace(GfxState *state) {
  writePSFmt("{0:.4g} Tw\n", (double)state->getWordSpace());
}
...
void PSOutputDev::updateTextPos(GfxState *state) {
  writePSFmt("{0:.4g} {1:.4g} Td\n", (double)state->getLineX(), (double)state->getLineY());
}

void PSOutputDev::updateTextShift(GfxState *state, FixedPoint shift) {
...
void PSOutputDev::tilingPatternFill(GfxState *state, Object *str,
  ...
  if (paintType == 2) {
    writePSFmt("{0:.4g} 0 {1:.4g} {2:.4g} {3:.4g} {4:.4g} setcachedevice\n",
	       (double)xStep, (double)bbox[0], (double)bbox[1], (double)bbox[2], (double)bbox[3]);
  } else {
    if (x1 - 1 <= x0) {
  ...
  writePSFmt("{0:d} 1 {1:d} {{ {2:.4g} exch {3:.4g} mul m {4:d} 1 {5:d} {{ pop (x) show }} for }} for\n",
	     y0, y1 - 1, x0 * (double)xStep, (double)yStep, x0, x1 - 1);
  writePS("grestore\n");
}
...
GBool PSOutputDev::axialShadedFill(GfxState *state, GfxAxialShading *shading) {
  ...
  FixedPoint x0, y0, x1, y1, dx, dy, mul;
  FixedPoint tMin, tMax, t, t0, t1;
  int i;
...
GBool PSOutputDev::radialShadedFill(GfxState *state,
  ...
  FixedPoint xMin, yMin, xMax, yMax;
  FixedPoint x0, y0, r0, x1, y1, r1, t0, t1;
  FixedPoint xa, ya, ra;
  FixedPoint sz, xz, yz, sMin, sMax, sa, ta;
  FixedPoint theta, alpha, a1, a2;
  FixedPoint V180_PI = (FixedPoint)(180 / M_PI);
  GBool enclosed;
  int i;
  ...
  if (enclosed) {
    ...
    a1 = V180_PI * (alpha + theta) + 90;
    a2 = V180_PI * (alpha - theta) - 90;
    while (a2 < a1) {
      a2 += 360;
    }
  }
  ...
  if (enclosed) {
	...
	writePSFmt("{0:.4g} radialCol sc\n", (double)ta);
      ...
      writePSFmt("{0:.4g} {1:.4g} m {2:.4g} {3:.4g} l {4:.4g} {5:.4g} l {6:.4g} {7:.4g} l h f*\n",
		 (double)xMin, (double)yMin, (double)xMin, (double)yMax, (double)xMax, (double)yMax, (double)xMax, (double)yMin);
    }
  }
...
void PSOutputDev::doPath(GfxPath *path) {
  GfxSubpath *subpath;
  FixedPoint x0, y0, x1, y1, x2, y2, x3, y3, x4, y4;
  int n, m, i, j;
  ...
  if (n == 1 && path->getSubpath(0)->getNumPoints() == 5) {
	...
	writePSFmt("{0:.4g} {1:.4g} {2:.4g} {3:.4g} re\n",
		   x0 < x2 ? (double)x0 : (double)x2, y0 < y1 ? (double)y0 : (double)y1,
		   fabs(x2 - x0), fabs(y1 - y0));
	return;
      } else if (x0 == x3 && x1 == x2 && y0 == y1 && y2 == y3) {
	writePSFmt("{0:.4g} {1:.4g} {2:.4g} {3:.4g} re\n",
		   x0 < x1 ? (double)x0 : (double)x1, y0 < y2 ? (double)y0 : (double)y2,
  ...
  for (i = 0; i < n; ++i) {
    ...
    writePSFmt("{0:.4g} {1:.4g} m\n", (double)subpath->getX(0), (double)subpath->getY(0));
    j = 1;
    while (j < m) {
      if (subpath->getCurve(j)) {
	writePSFmt("{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} c\n",
		   (double)subpath->getX(j), (double)subpath->getY(j),
		   (double)subpath->getX(j+1), (double)subpath->getY(j+1),
		   (double)subpath->getX(j+2),(double) subpath->getY(j+2));
	j += 3;
      } else {
...
void PSOutputDev::drawString(GfxState *state, GooString *s) {
  ...
  Gushort *codeToGID;
  GooString *s2;
  FixedPoint dx, dy, dx2, dy2, originX, originY;
  char *p;
  UnicodeMap *uMap;
  CharCode code;
  Unicode *u;
...
void PSOutputDev::doImageL2(Object *ref, GfxImageColorMap *colorMap,
  ...
  if (colorMap) {
	...
	colorMap->getColorSpace()->getMode() == csSeparation) {
      ...
      writePSFmt("{0:.4g} {1:.4g}", (double)(colorMap->getDecodeLow(0) * n),
		 (double)(colorMap->getDecodeHigh(0) * n));
	...
	writePSFmt("{0:.4g} {1:.4g}",
		   (double)colorMap->getDecodeLow(i), (double)colorMap->getDecodeHigh(i));
      }
    }
    writePS("]\n");
  } else {
...
void PSOutputDev::doImageL3(Object *ref, GfxImageColorMap *colorMap,
  ...
  if (colorMap) {
	...
	writePSFmt("{0:.4g} {1:.4g}", (double)colorMap->getDecodeLow(i),
		   (double)colorMap->getDecodeHigh(i));
      }
    }
    writePS("]\n");
  } else {
...
void PSOutputDev::dumpColorSpaceL2(GfxColorSpace *colorSpace,
  ...
  case csCalGray:
    ...
    writePS("[/CIEBasedA <<\n");
    writePSFmt(" /DecodeA {{{0:.4g} exp}} bind\n", (double)calGrayCS->getGamma());
    writePSFmt(" /MatrixA [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)calGrayCS->getWhiteX(), (double)calGrayCS->getWhiteY(),
	       (double)calGrayCS->getWhiteZ());
    writePSFmt(" /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)calGrayCS->getWhiteX(), (double)calGrayCS->getWhiteY(),
	       (double)calGrayCS->getWhiteZ());
    writePSFmt(" /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)calGrayCS->getBlackX(), (double)calGrayCS->getBlackY(),
	       (double)calGrayCS->getBlackZ());
    writePS(">>]");
    if (genXform) {
      writePS(" {}");
    }
  ...
  case csCalRGB:
    ...
    writePSFmt(" /DecodeABC [{{{0:.4g} exp}} bind {{{1:.4g} exp}} bind {{{2:.4g} exp}} bind]\n",
	       (double)calRGBCS->getGammaR(), (double)calRGBCS->getGammaG(),
	       (double)calRGBCS->getGammaB());
    writePSFmt(" /MatrixABC [{0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} {6:.4g} {7:.4g} {8:.4g}]\n",
	       (double)calRGBCS->getMatrix()[0], (double)calRGBCS->getMatrix()[1],
	       (double)calRGBCS->getMatrix()[2], (double)calRGBCS->getMatrix()[3],
	       (double)calRGBCS->getMatrix()[4], (double)calRGBCS->getMatrix()[5],
	       (double)calRGBCS->getMatrix()[6], (double)calRGBCS->getMatrix()[7],
	       (double)calRGBCS->getMatrix()[8]);
    writePSFmt(" /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)calRGBCS->getWhiteX(), (double)calRGBCS->getWhiteY(),
	       (double)calRGBCS->getWhiteZ());
    writePSFmt(" /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)calRGBCS->getBlackX(), (double)calRGBCS->getBlackY(),
	       (double)calRGBCS->getBlackZ());
    writePS(">>]");
    if (genXform) {
      writePS(" {}");
    }
  ...
  case csLab:
    ...
    if (map01) {
      ...
      writePSFmt(" /DecodeABC [{{100 mul 16 add 116 div}} bind {{{0:.4g} mul {1:.4g} add}} bind {{{2:.4g} mul {3:.4g} add}} bind]\n",
		 (double)(labCS->getAMax() - labCS->getAMin()) / 500.0,
		 (double)labCS->getAMin() / 500.0,
		 (double)(labCS->getBMax() - labCS->getBMin()) / 200.0,
		 (double)labCS->getBMin() / 200.0);
    } else {
      writePSFmt(" /RangeABC [0 100 {0:.4g} {1:.4g} {2:.4g} {3:.4g}]\n",
		 (double)labCS->getAMin(), (double)labCS->getAMax(),
		 (double)labCS->getBMin(), (double)labCS->getBMax());
    ...
    writePSFmt("     {{4 29 div sub 108 841 div mul }} ifelse {0:.4g} mul}} bind\n",
	       (double)labCS->getWhiteX());
    writePS("    {dup 6 29 div ge {dup dup mul mul}\n");
    writePSFmt("     {{4 29 div sub 108 841 div mul }} ifelse {0:.4g} mul}} bind\n",
	       (double)labCS->getWhiteY());
    writePS("    {dup 6 29 div ge {dup dup mul mul}\n");
    writePSFmt("     {{4 29 div sub 108 841 div mul }} ifelse {0:.4g} mul}} bind]\n",
	       (double)labCS->getWhiteZ());
    writePSFmt(" /WhitePoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)labCS->getWhiteX(), (double)labCS->getWhiteY(), (double)labCS->getWhiteZ());
    writePSFmt(" /BlackPoint [{0:.4g} {1:.4g} {2:.4g}]\n",
	       (double)labCS->getBlackX(), (double)labCS->getBlackY(), (double)labCS->getBlackZ());
    writePS(">>]");
    if (genXform) {
      writePS(" {}");
    }
  ...
  case csIndexed:
	...
	for (j = i; j < i+8 && j <= n; ++j) {
	  for (k = 0; k < numComps; ++k) {
	    x[k] = low[k] + (FixedPoint)(*p++ / 255.0) * range[k];
	  }
	  func->transform(x, y);
	  if (labCS) {
	    y[0] /= 100.0;
...
void PSOutputDev::opiBegin20(GfxState *state, Dict *dict) {
  ...
  FixedPoint width, height, left, right, top, bottom;
  int w, h;
  int i;
...
void PSOutputDev::opiBegin13(GfxState *state, Dict *dict) {
  ...
  FixedPoint c, m, y, k;
  FixedPoint llx, lly, ulx, uly, urx, ury, lrx, lry;
  FixedPoint tllx, tlly, tulx, tuly, turx, tury, tlrx, tlry;
  FixedPoint horiz, vert;
  int i, j;
  ...
  if (obj1.isArray() && obj1.arrayGetLength() == 8) {
    ...
    writePSFmt("%ALDImagePosition: {0:.4g} {1:.4g} {2:.4g} {3:.4g} {4:.4g} {5:.4g} {6:.4g} {7:.4g}\n",
	       (double)tllx, (double)tlly, (double)tulx, (double)tuly, (double)turx, (double)tury, (double)tlrx, (double)tlry);
    obj2.free();
  }
  obj1.free();
...
void PSOutputDev::opiTransform(GfxState *state, FixedPoint x0, FixedPoint y0,
			       FixedPoint *x1, FixedPoint *y1) {
  FixedPoint t;
...
void PSOutputDev::type3D0(GfxState *state, FixedPoint wx, FixedPoint wy) {
  writePSFmt("{0:.4g} {1:.4g} setcharwidth\n", (double)wx, (double)wy);
  writePS("q\n");
  t3NeedsRestore = gTrue;
}

void PSOutputDev::type3D1(GfxState *state, FixedPoint wx, FixedPoint wy,
...
void PSOutputDev::cvtFunction(Function *func) {
  ...
  case 0:			// sampled
    ...
    for (i = m-1; i >= 0; --i) {
      ...
      writePSFmt("{0:.4g} sub {1:.4g} mul {2:.4g} add\n",
	      (double)func0->getDomainMin(i),
	      (double)((func0->getEncodeMax(i) - func0->getEncodeMin(i)) /
	        (func0->getDomainMax(i) - func0->getDomainMin(i))),
	      (double)func0->getEncodeMin(i));
	...
	// [e01] [efrac] s'(0) s'(1) ... s(2^(m-j-1)-1)
      ...
      writePSFmt("{0:.4g} mul {1:.4g} add\n",
		 (double)(func0->getDecodeMax(i) - func0->getDecodeMin(i)),
		 (double)func0->getDecodeMin(i));
      writePSFmt("dup {0:.4g} lt {{ pop {1:.4g} }} {{ dup {2:.4g} gt {{ pop {3:.4g} }} if }} ifelse\n",
		 (double)func0->getRangeMin(i), (double)func0->getRangeMin(i),
		 (double)func0->getRangeMax(i), (double)func0->getRangeMax(i));
  ...
  case 2:			// exponential
    ...
    writePSFmt("{{ dup {0:.4g} lt {{ pop {1:.4g} }} {{ dup {2:.4g} gt {{ pop {3:.4g} }} if }} ifelse\n",
	       (double)func2->getDomainMin(0), (double)func2->getDomainMin(0),
	       (double)func2->getDomainMax(0), (double)func2->getDomainMax(0));
    // x
    for (i = 0; i < n; ++i) {
      // x y(0) .. y(i-1)
      writePSFmt("{0:d} index {1:.4g} exp {2:.4g} mul {3:.4g} add\n",
		 i, (double)func2->getE(), (double)(func2->getC1()[i] - func2->getC0()[i]),
		 (double)func2->getC0()[i]);
      if (func2->getHasRange()) {
	writePSFmt("dup {0:.4g} lt {{ pop {1:.4g} }} {{ dup {2:.4g} gt {{ pop {3:.4g} }} if }} ifelse\n",
		   (double)func2->getRangeMin(i), (double)func2->getRangeMin(i),
		   (double)func2->getRangeMax(i), (double)func2->getRangeMax(i));
      }
    }
    // x y(0) .. y(n-1)
  ...
  case 3:			// stitching
    ...
    writePSFmt("{{ dup {0:.4g} lt {{ pop {1:.4g} }} {{ dup {2:.4g} gt {{ pop {3:.4g} }} if }} ifelse\n",
	       (double)func3->getDomainMin(0), (double)func3->getDomainMin(0),
	       (double)func3->getDomainMax(0), (double)func3->getDomainMax(0));
    for (i = 0; i < func3->getNumFuncs() - 1; ++i) {
      writePSFmt("dup {0:.4g} lt {{ {1:.4g} sub {2:.4g} mul {3:.4g} add xpdfFunc{4:d}_{5:d} }} {{\n",
		 (double)func3->getBounds()[i+1],
		 (double)func3->getBounds()[i],
		 (double)func3->getScale()[i],
		 (double)func3->getEncode()[2*i],
		 thisFunc, i);
    }

...
void IdentityFunction::transform(FixedPoint *in, FixedPoint *out) {
  int i;
...
SampledFunction::SampledFunction(Object *funcObj, Dict *dict) {
  ...
  //----- buffer
  sBuf = (FixedPoint *)gmallocn(1 << m, sizeof(FixedPoint));
  ...
  sampleMul = 1.0 / (pow(2.0, (FixedPoint)sampleBits) - 1);
  obj1.free();
  ...
  samples = (FixedPoint *)gmallocn(nSamples, sizeof(FixedPoint));
  buf = 0;
  bits = 0;
  ...
  for (i = 0; i < nSamples; ++i) {
    ...
    samples[i] = (FixedPoint)(int)s * sampleMul;
  }
  str->close();
...
SampledFunction::SampledFunction(SampledFunction *func) {
  ...
  samples = (FixedPoint *)gmallocn(nSamples, sizeof(FixedPoint));
  memcpy(samples, func->samples, nSamples * sizeof(FixedPoint));
  sBuf = (FixedPoint *)gmallocn(1 << m, sizeof(FixedPoint));
}

void SampledFunction::transform(FixedPoint *in, FixedPoint *out) {
  FixedPoint x;
  int e[funcMaxInputs][2];
  FixedPoint efrac0[funcMaxInputs];
  FixedPoint efrac1[funcMaxInputs];
  int i, j, k, idx, t;
  FixedPoint ONE = (FixedPoint)1;
...
void ExponentialFunction::transform(FixedPoint *in, FixedPoint *out) {
  FixedPoint x;
  int i;
  ...
  for (i = 0; i < n; ++i) {
    out[i] = c0[i] + FixedPoint::pow(x, e) * (c1[i] - c0[i]);
    if (hasRange) {
      if (out[i] < range[i][0]) {
	out[i] = range[i][0];
      } else if (out[i] > range[i][1]) {
...
StitchingFunction::StitchingFunction(Object *funcObj, Dict *dict) {
  ...
  bounds = (FixedPoint *)gmallocn(k + 1, sizeof(FixedPoint));
  encode = (FixedPoint *)gmallocn(2 * k, sizeof(FixedPoint));
  scale = (FixedPoint *)gmallocn(k, sizeof(FixedPoint));
  for (i = 0; i < k; ++i) {
    funcs[i] = NULL;
  }
  for (i = 0; i < k; ++i) {
...
StitchingFunction::StitchingFunction(StitchingFunction *func) {
  ...
  bounds = (FixedPoint *)gmallocn(k + 1, sizeof(FixedPoint));
  memcpy(bounds, func->bounds, (k + 1) * sizeof(FixedPoint));
  encode = (FixedPoint *)gmallocn(2 * k, sizeof(FixedPoint));
  memcpy(encode, func->encode, 2 * k * sizeof(FixedPoint));
  scale = (FixedPoint *)gmallocn(k, sizeof(FixedPoint));
  memcpy(scale, func->scale, k * sizeof(FixedPoint));
  ok = gTrue;
}
...
void StitchingFunction::transform(FixedPoint *in, FixedPoint *out) {
  FixedPoint x;
  int i;
...
public:
  ...
  void pushReal(FixedPoint real);
  void pushRawReal(int real);
  GBool popBool();
  int popInt();
  FixedPoint popNum();
...
void PSStack::pushReal(FixedPoint real) {
  if (checkOverflow()) {
    stack[--sp].type = psReal;
    stack[sp].real = real.getRaw();
  }
}

void PSStack::pushRawReal(int real) {
  if (checkOverflow()) {
    stack[--sp].type = psReal;
    stack[sp].real = real;
  }
...
FixedPoint PSStack::popNum() {
  FixedPoint ret;

  if (checkUnderflow() && checkType(psInt, psReal)) {
    ret = (stack[sp].type == psInt) ? (FixedPoint)stack[sp].intg : FixedPoint::make(stack[sp].real);
    ++sp;
    return ret;
  }
  return 0;
...
void PostScriptFunction::transform(FixedPoint *in, FixedPoint *out) {
  PSStack *stack;
  int i;
...
GBool PostScriptFunction::parseCode(Stream *str, int *codePtr) {
  ...
  while (1) {
	...
	code[*codePtr].type = psReal;
        code[*codePtr].real = ((FixedPoint)(atof(tok->getCString()))).getRaw();
      } else {
...
void PostScriptFunction::exec(PSStack *stack, int codePtr) {
  ...
  while (1) {
    ...
    case psInt:
      ...
      break;
    case psReal:
      stack->pushRawReal(code[codePtr++].real);
      break;
    case psOperator:
	...
	if (stack->topIsInt()) {
	  ...
	  stack->pushReal(FixedPoint::abs(stack->popNum()));
	}
	break;
      case psOpAdd:
	...
	break;
      case psOpCeiling:
	if (!stack->topIsInt()) {
	  stack->pushReal(FixedPoint::ceil(stack->popNum()));
	}
	break;
      case psOpCopy:
	...
	r2 = stack->popNum();
	r1 = stack->popNum();
	stack->pushReal(FixedPoint::pow(r1, r2));
	break;
      case psOpFalse:

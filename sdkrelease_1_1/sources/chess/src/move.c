   ...
   if (ATOH (mvstr[0]))	/* pawn move */
	{
	 if (ITO8 (mvstr[1]))					/* e4 type */
		...
		if (ATOH (mvstr[1]) && ITO8 (mvstr[2]))		/* ed4 type */
			{
...
*/
   	...
   	if ((piece = piece_id(mvstr[0])) != empty && (piece_id(mvstr[1]) == empty))	/* Is a piece move */
   		{
...
{
   leaf *node;
   if ((PromotionFlag==1) &&(SelectPromotion==0))
   	SetPromotion();
   if (SelectPromotion>5)
   	{
   	 piece=SelectPromotion;
   	 PromotionFlag=0;
   	 SelectPromotion=0;
   	}
   for (node = TreePtr[ply]; node < TreePtr[ply + 1]; node++)
	 if ((int) (node->move & 0x0FFF) == MOVE(f,t)  && toupper(piece) == notation[PROMOTEPIECE (node->move)])
	 	return (node);
   return ((leaf *) NULL);
}

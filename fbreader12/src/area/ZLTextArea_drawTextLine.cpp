...
#include "../nanox/image/ZLNXImageManager.h"
...
void ZLTextArea::drawTextLine(Style &style, const ZLTextLineInfo &info, int y, size_t from, size_t to) {
	...
	if (!mySelectionModel.isNull() && !mySelectionModel->isMarkerEmpty() && (from != to)) {
		const std::vector<ZLTextSelectionModel::Range> &ranges = mySelectionModel->MarkerRanges();
		if (!ranges.empty()) {
		...
		if ((kind == ZLTextElement::WORD_ELEMENT) || (kind == ZLTextElement::IMAGE_ELEMENT)) {
			...
			if (kind == ZLTextElement::WORD_ELEMENT) {
				drawWord(style, wx, wy, (const ZLTextWord&)element, it->StartCharIndex, -1, false);
					...
					SynWordList *wp = &(acwordlist[acwlistcount++]);
					wp->type = 1;
					if (it->BidiLevel == 0)
					{
						wp->x1 = imgposx + it->XStart;
						wp->x2 = imgposx + it->XEnd;
					}
					else
					{
						int w = it->XEnd - it->XStart + 1;
						int mirrorx = realX(it->XStart) - w;
						wp->x1 = imgposx + mirrorx;
						wp->x2 = imgposx + mirrorx + w;
					}
					wp->y1 = imgposy + it->YEnd - style.elementHeight(element, metrics) - style.elementDescent(element) - style.textStyle()->verticalShift();
					wp->y2 = imgposy + it->YEnd;
					wp->pnum = paragraph.index();
					wp->wnum = it->ElementIndex;
					wp->word = (char *) malloc(word.Size+1);
					memcpy(wp->word, word.Data, word.Size);
					wp->word[word.Size] = 0;
					acwordlist[acwlistcount].word = NULL;
				...
				ZLNXImageData &nximage = (ZLNXImageData &)(*((const ZLTextImageElement&)element).image());
				iH = nximage.height();
			}
		}
	}
	if (it != toIt) {
		...
		drawWord(style, x, y, word, start, len, it->AddHyphenationSign);
			...
			wp->y1 = imgposy + it->YEnd - style.elementHeight(info.End.element(), metrics) - style.elementDescent(info.End.element()) - style.textStyle()->verticalShift();
			wp->y2 = imgposy + it->YEnd;
			wp->pnum = paragraph.index();
			wp->wnum = info.End.elementIndex();
			wp->word = (char*)malloc(byte_len + 1);
			memcpy(wp->word, word.Data + start, byte_len);
			wp->word[byte_len] = 0;
			acwordlist[acwlistcount].word = NULL;
	}

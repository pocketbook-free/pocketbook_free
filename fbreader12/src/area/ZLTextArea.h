...
public:
	...
	// PB
	bool _getHyperlinkInfo(const ZLTextElementRectangle &rectangle, std::string &id, std::string &type, ZLTextKind& kind, int& x1, int&y1, int& x2, int& y2) const;
	int get_xxx_links(std::vector<struct xxx_link> & links) const;

...
public:
	void AddMarker(int startParagraphIndex, int startElementIndex, int startCharIndex,
			int endParagraphIndex, int endElementIndex, int endCharIndex);
	void ClearMarkers();
	bool isMarkerEmpty() const;
	const std::vector<Range> &MarkerRanges() const;
	void SetSelection(int startParagraphIndex, int startElementIndex, int startCharIndex,
			int endParagraphIndex, int endElementIndex, int endCharIndex);
	void GetSelection(int *startParagraphIndex, int *startElementIndex, int *startCharIndex,
			int *endParagraphIndex, int *endElementIndex, int *endCharIndex);
	void GetCoordByPos(int pi, int ei, int *x1, int *y1, int *x2, int *y2);

private:
	bool myIsMarkerEmpty;
	std::vector<Bound> myMarkerBegin;
	std::vector<Bound> myMarkerEnd;

	mutable std::vector<Range> myMarkerRanges;
	mutable bool myMarkerRangeVectorIsUpToDate;

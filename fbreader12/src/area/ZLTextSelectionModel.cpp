	...
	if (it != myArea.myTextElementMap.end()) {
		...
		if (ZLTextElementRectangle::RangeChecker(x1, y1)(*it)) {
			...
			if (it->Kind == ZLTextElement::WORD_ELEMENT) {
				bound.After.CharIndex = 0;
				bound.Before.CharIndex = it->Length;
			}
...
void ZLTextSelectionModel::AddMarker(int startParagraphIndex, int startElementIndex, int startCharIndex,
					int endParagraphIndex, int endElementIndex, int endCharIndex)
{
	Bound start;
	Bound end;
	start.After.ParagraphIndex = startParagraphIndex;
	start.After.ElementIndex = startElementIndex;
	start.After.CharIndex = startCharIndex;
	start.After.Exists = true;
	start.Before.Exists = false;

	end.After.ParagraphIndex = endParagraphIndex;
	end.After.ElementIndex = endElementIndex;
	end.After.CharIndex = endCharIndex;
	end.After.Exists = true;
	end.Before.Exists = false;

	myMarkerBegin.push_back(start);
	myMarkerEnd.push_back(end);

	myIsMarkerEmpty = false;
	myMarkerRanges.clear();
	myMarkerRangeVectorIsUpToDate = false;
}

void ZLTextSelectionModel::ClearMarkers()
{
	myMarkerBegin.clear();
	myMarkerEnd.clear();
	myIsMarkerEmpty = true;
	myMarkerRanges.clear();
	myMarkerRangeVectorIsUpToDate = false;
}

bool ZLTextSelectionModel::isMarkerEmpty() const
{
	return myIsMarkerEmpty;
//	if (myIsMarkerEmpty)
//		return true;
//	Range r = internalRange();
//	return !r.first.Exists || !r.second.Exists || (r.first == r.second);
...
const std::vector<ZLTextSelectionModel::Range> &ZLTextSelectionModel::MarkerRanges() const {
	if (!myMarkerRangeVectorIsUpToDate && !isMarkerEmpty()) {
		int MarkerIter;
		int MarkerSize = myMarkerBegin.size();
		for (MarkerIter = 0; MarkerIter < MarkerSize; MarkerIter++)
		{
			Range r = Range(myMarkerBegin[MarkerIter].After, myMarkerEnd[MarkerIter].After);
			...
			if ((startLevel == 0) && (endLevel == 0)) {
				...
				for (int i = r.first.ElementIndex; i < r.second.ElementIndex; ++i) {
					...
					if ((kind == ZLTextElement::START_REVERSED_SEQUENCE_ELEMENT) ||
							(kind == ZLTextElement::END_REVERSED_SEQUENCE_ELEMENT)) {
						rightBound.ElementIndex = i;
						myMarkerRanges.push_back(Range(leftBound, rightBound));
						leftBound = rightBound;
					}
					cursor.nextWord();
				}
				myMarkerRanges.push_back(Range(leftBound, r.second));
			} else {
				BoundElement leftBound = r.first;
				if (startLevel > 0) {
					BoundElement rightBound;
					...
					while (endLevel > 0) {
						switch(cursor.element().kind()) {
							case ZLTextElement::START_REVERSED_SEQUENCE_ELEMENT:
								--endLevel;
								leftBound1.ElementIndex = cursor.elementIndex();
								myMarkerRanges.push_back(Range(leftBound1, rightBound1));
								rightBound1 = leftBound1;
								break;
							case ZLTextElement::END_REVERSED_SEQUENCE_ELEMENT:
								++endLevel;
				...
				myMarkerRanges.push_back(Range(leftBound, rightBound1));
			}
		}

		myMarkerRangeVectorIsUpToDate = true;
	}
	return myMarkerRanges;
}

void ZLTextSelectionModel::SetSelection(int startParagraphIndex, int startElementIndex, int startCharIndex,
						int endParagraphIndex, int endElementIndex, int endCharIndex)
{
		myFirstBound.After.ParagraphIndex = startParagraphIndex;
		myFirstBound.After.ElementIndex = startElementIndex;
		myFirstBound.After.CharIndex = startCharIndex;

		mySecondBound.Before.ParagraphIndex = endParagraphIndex;
		mySecondBound.Before.ElementIndex = endElementIndex;
		mySecondBound.Before.CharIndex = endCharIndex;
		myTextIsUpToDate = false;
		clearData();
		myRangeVectorIsUpToDate = false;
		myRanges.clear();
...
void ZLTextSelectionModel::GetSelection(int *startParagraphIndex, int *startElementIndex, int *startCharIndex,
						int *endParagraphIndex, int *endElementIndex, int *endCharIndex)
{
	if (mySecondBound < myFirstBound)
	{
		if (mySecondBound.After.Exists)
		{
			*startParagraphIndex = mySecondBound.After.ParagraphIndex;
			*startElementIndex = mySecondBound.After.ElementIndex;
			*startCharIndex = mySecondBound.After.CharIndex;
		}
		else
		{
			*startParagraphIndex = mySecondBound.Before.ParagraphIndex;
			*startElementIndex = mySecondBound.Before.ElementIndex;
			*startCharIndex = mySecondBound.Before.CharIndex;
		}

		if (myFirstBound.Before.Exists)
		{
			*endParagraphIndex = myFirstBound.Before.ParagraphIndex;
			*endElementIndex = myFirstBound.Before.ElementIndex;
			*endCharIndex = myFirstBound.Before.CharIndex;
		}
		else
		{
			*endParagraphIndex = myFirstBound.After.ParagraphIndex;
			*endElementIndex = myFirstBound.After.ElementIndex;
			*endCharIndex = myFirstBound.After.CharIndex;
		}
	}
	else
	{
		if (myFirstBound.After.Exists)
		{
			*startParagraphIndex = myFirstBound.After.ParagraphIndex;
			*startElementIndex = myFirstBound.After.ElementIndex;
			*startCharIndex = myFirstBound.After.CharIndex;
		}
		else
		{
			*startParagraphIndex = myFirstBound.Before.ParagraphIndex;
			*startElementIndex = myFirstBound.Before.ElementIndex;
			*startCharIndex = myFirstBound.Before.CharIndex;
		}

		if (mySecondBound.Before.Exists)
		{
			*endParagraphIndex = mySecondBound.Before.ParagraphIndex;
			*endElementIndex = mySecondBound.Before.ElementIndex;
			*endCharIndex = mySecondBound.Before.CharIndex;
		}
		else
		{
			*endParagraphIndex = mySecondBound.After.ParagraphIndex;
			*endElementIndex = mySecondBound.After.ElementIndex;
			*endCharIndex = mySecondBound.After.CharIndex;
		}
	}

}

void ZLTextSelectionModel::GetCoordByPos(int pi, int ei, int *x1, int *y1, int *x2, int *y2)
{
	if (myArea.myTextElementMap.empty())
		return;
	...
	ZLTextElementMap::const_iterator it = myArea.myTextElementMap.begin();
	for (; it != myArea.myTextElementMap.end(); ++it)
		if (it->ParagraphIndex == pi && it->ElementIndex == ei)
			break;

	if (it != myArea.myTextElementMap.end())
	{
		*x1 = it->XStart + myArea.hOffset();
		*x2 = it->XEnd + myArea.hOffset();
		*y1 = it->YStart + myArea.vOffset();
		*y2 = it->YEnd + myArea.vOffset();
	}
}

...
std::string FBReader::EncodingOverride; // PB: encoding from user settings
std::string FBReader::LanguageOverride; // PB: language from user settings
...
bool FBReader::calc_in_progress = false;
...
bool FBReader::isBookReady(){
	return myBookReady;
}

bool FBReader::getGlobalAllowHyphenations(){
	return myGlobalAllowHyphenations;
}

void FBReader::setGlobalAllowHyphenations(bool value){
	myGlobalAllowHyphenations = value;
}

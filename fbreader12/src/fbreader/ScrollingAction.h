...
class ForwardScrollingAction {
    static const int checkPagesCount = 30;
public:
        //returns true when end
        static bool run(ZLTextView & view, long long & cp, int & calc_current_page, int & npages, TPageList & pageList, TPageList & oldPageList, unsigned int TTP, bool & areEqual);

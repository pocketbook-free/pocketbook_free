...
bool ZLTextPartialDecoratedStyle::allowHyphenations() const {
	...
	bool fbValue = (a == B3_UNDEFINED) ? base()->allowHyphenations() : (a == B3_TRUE);
	// check value from the device settings
	bool pbValue = FBReader::Instance().getGlobalAllowHyphenations();
	return fbValue && pbValue;
}

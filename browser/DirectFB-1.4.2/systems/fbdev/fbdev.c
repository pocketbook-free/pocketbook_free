...
/******************************************************************************/
static unsigned char video_buffer[2*1600*1600] = {0};
...
static DFBResult dfb_fbdev_open( void )
{
     ...
     if (dfb_config->fb_device) {
          dfb_fbdev->fd = open( dfb_config->fb_device, O_RDWR | O_NONBLOCK);
          if (dfb_fbdev->fd < 0) {
...
system_initialize( CoreDFB *core, void **data )
{
     ...
     /* Map the framebuffer */
     dfb_fbdev->framebuffer_base = video_buffer;
...
init_modes( void )
{
     ...
     if (!dfb_fbdev->shared->modes) {
	  ...
	  /*dfb_fbdev->shared->modes->next = (VideoMode*) SHCALLOC( dfb_fbdev->shared->shmpool, 1, sizeof(VideoMode) );
	  direct_memcpy (dfb_fbdev->shared->modes->next, dfb_fbdev->shared->modes, sizeof(VideoMode));
	  dfb_fbdev->shared->modes->next->next = NULL;
	  dfb_fbdev->shared->modes->next->xres = dfb_fbdev->shared->modes->yres;
	  dfb_fbdev->shared->modes->next->yres = dfb_fbdev->shared->modes->xres;*/
                  ...
                  DFBColorAdjustment         *adjustment )
{
     ...
     dfb_config->mode.format = DSPF_RGB332;
                       ...
                       struct fb_var_screeninfo  *ret_var )
{
     ...
     D_INFO( "FBDev_Mode  -> resolution   %dx%d\n", mode->xres, mode->yres );
     D_INFO( "FBDev_Mode  -> virtual      %dx%d\n", vxres, vyres );
     D_INFO( "FBDev_Mode  -> pixelformat  %s\n", dfb_pixelformat_name(pixelformat) );
     D_INFO( "FBDev_Mode  -> buffermode   %s\n",
                 buffermode == DLBM_FRONTONLY  ? "FRONTONLY"  :
                 buffermode == DLBM_BACKVIDEO  ? "BACKVIDEO"  :
                 buffermode == DLBM_BACKSYSTEM ? "BACKSYSTEM" :
                 buffermode == DLBM_TRIPLE     ? "TRIPLE"     : "invalid!" );
                     ...
                     const CoreLayerRegionConfig *config )
{
     ...
     /* Enable test mode */
     //TODO: sbelov commented var.activate = FB_ACTIVATE_TEST;
                    ...
                    unsigned int             yoffset )
{
     ...
     shared->fix.line_length = var.xres;

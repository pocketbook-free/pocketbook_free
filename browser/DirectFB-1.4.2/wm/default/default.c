               ...
               DFBSurfaceFlipFlags  flags )
{
     ...
     for (i=0; i<num_updates; i++) {
	  ...
	  const int rotation = stack->context->rotation;
	  ...
	  {
	       if (!dfb_region_intersect( &dest, 0, 0, surface->config.size.h - 1, surface->config.size.w - 1 ))
                   continue;
	  }
	  else*/
	  {
               ...
               const DFBInputEvent *event )
{
     ...
     fprintf(stderr, "WM: event recieved\n");
     ...
     switch (DFB_LOWER_CASE(event->key_symbol)) {
	  ...
	  case DIKS_SMALL_L:
		   ...
		   switch ((stack->context->rotation + 90) % 360) {
	               default:
	               case 0:
		       case 180:
		           size.w = stack->context->config.width;
		           size.h = stack->context->config.height;
		           break;
		       case 90:
		       case 270:
		           size.w = stack->context->config.height;
		           size.h = stack->context->config.width;
		           break;
	           }
		   ...
		   //dfb_layer_context_set_configuration( stack->context, &config );
		   dfb_layer_context_set_rotation(stack->context, (stack->context->rotation + 90) % 360);		   
		   
		   fusion_vector_foreach (window, i, data->windows) {
			if (VISIBLE_WINDOW(window))
			{
			    window->config.bounds.w = size.w;
			    window->config.bounds.h = size.h;
			    
			    window->surface->config.size.w = size.w;
			    window->surface->config.size.h = size.h;
			    dfb_surface_notify(window->surface, CSNF_SIZEFORMAT);
                  ...
                  const DFBInputEvent *event )
{
     ...
     fprintf(stderr, "WM: handle_key_press key recieved\n");
     if (data->wm_level) {
                    ...
                    const DFBInputEvent *event )
{
     ...
     fprintf(stderr, "WM: handle_key_release key recieved\n");
     if (data->wm_level) {
          switch (event->key_symbol) {
               case DIKS_META:
                    data->wm_level &= ~1;

...
static void
add_pocketbook_cookie(SoupCookieJar* jar, const char *domain)
{
    SoupCookie* cookie;

    gchar* envSerial = g_strdup(GetSerialNumber());
    cookie = soup_cookie_new("PBSERIAL", envSerial, domain, "/", /*-1*/ SOUP_COOKIE_MAX_AGE_ONE_YEAR);
    if (cookie)
	soup_cookie_jar_add_cookie(jar, cookie);

    gchar *softVersion = g_strdup(GetSoftwareVersion());
    cookie = soup_cookie_new("PBFIRMWARE", softVersion, domain, "/", /*-1*/ SOUP_COOKIE_MAX_AGE_ONE_YEAR);
    if (cookie)
	soup_cookie_jar_add_cookie(jar, cookie);
}
                 ...
                 const gchar*   filename)
{
    ...
    SoupCookie* cookie;
    iv_netcookie cookie_rec;

    memset(&cookie_rec, 0, sizeof(cookie_rec));

    while(EnumCookie(&cookie_rec))
    {
	cookie = soup_cookie_new(cookie_rec.name, cookie_rec.value, cookie_rec.domain, cookie_rec.path, SOUP_COOKIE_MAX_AGE_ONE_YEAR);
	if (cookie)
		soup_cookie_jar_add_cookie(jar, cookie);
    }

    EnumCookieFree(&cookie_rec);

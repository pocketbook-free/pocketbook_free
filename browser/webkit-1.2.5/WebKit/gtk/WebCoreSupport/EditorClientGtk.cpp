...
void webkit_virtualkbd2_innertext_handler(char *text){ }
...
void EditorClient::handleKeyboardEvent(KeyboardEvent* event)
{
    ...
    if (editorCommandString) {
	    ...
	    else if (command.isTextInsertion() && 
		event->type() == eventNames().keydownEvent && event->keyCode() == 13 && 
		!(ivstate.uiflags & IN_KEYBOARD))
	    {
		...
		frame->editor()->command("MoveToBeginningOfLine").execute();
	    	...
	    	if (NULL != frame->selectedText().utf8().data())
	    	{
			strncpy(&text_elem[0], frame->selectedText().utf8().data(), 1023);
	    	}
	    	OpenKeyboard("", text_elem, 1023, KBD_ENTEXT, webkit_virtualkbd2_innertext_handler);
	   	while (ivstate.uiflags & IN_KEYBOARD)
	    	{
			sleep(1);
	    	}
	    	frame->editor()->command("SelectLine").execute();
	    	frame->editor()->command("Delete").execute();
	    	if ( strlen(text_elem) > 0 )
	    	{
			  frame->editor()->command("InsertText").execute(String::fromUTF8(text_elem));
	   	}
		isEntered = 1;
	    }

...
gail_entry_finalize (GObject            *object)
{
  ...
  if (entry->delete_idle_handler)
    {
      g_source_remove (entry->delete_idle_handler);
                            ...
                            gint      arg2)
{
   ...
   * or in an idle handler if it not updated.
   */
   if (gail_entry->delete_idle_handler == 0)
     gail_entry->delete_idle_handler = g_idle_add (idle_notify_delete, gail_entry);
}

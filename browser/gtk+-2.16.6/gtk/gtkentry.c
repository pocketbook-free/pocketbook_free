...
void gtk_virtualkbd_entry_handler(char *text){
}
			...
			GdkEventButton *event)
{
  ...
  if (event->button == 1)
	...
	case GDK_BUTTON_PRESS:
	  ...
	  {
	      gchar text_value[1024] = {0};
	      GtkClipboard *clipboard = NULL;
	      
	      strncpy(text_value, GTK_EDITABLE_GET_CLASS (editable)->get_chars (editable, 0, -1), 1024);
	      OpenKeyboard("", text_value, 1024, KBD_URL, gtk_virtualkbd_entry_handler);
	      while (ivstate.uiflags & IN_KEYBOARD)
	      {
		  sleep(1);
	      }
	      clipboard = gtk_widget_get_clipboard (GTK_WIDGET (editable), GDK_SELECTION_CLIPBOARD);
	      gtk_editable_delete_text (editable, 0, -1);
	      gtk_clipboard_clear (clipboard);
	      gtk_clipboard_set_text (clipboard, _(text_value), -1);

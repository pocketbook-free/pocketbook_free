 ...
 */
 extern void set_global_supplies(int);
...
{	
	...
	if((usb_status & 0xc0000) == (0x3 << 18)) 
	{
		if(g_reset_available) 
		{
			...
			dev->ep0state = WAIT_FOR_SETUP;
			g_reset_available = 0;
			set_global_supplies(2);		//Henry Li:  MAX8698_USB_SUPPLY
...
static irqreturn_t s3c_udc_irq(int irq, void *_dev)
{
	...
	//{
	// writel(INI_OUT_EP,S3C_UDC_OTG_GINTSTS);
	//}
	...
	if( (intr_status & 0xffff) == 0x2020)		//* Henry Li: 20101112 */
	{
	...
	if (intr_status & INT_ENUMDONE) 
	{
		...
		if( (intr_status & 0xffff) == 0x2020)		//* Henry Li: 20101112 */
			{
			udc_cable_status=STATUS_UDC_CONNECT;
			usb_connection_mode = PC_LINK;			
			}
		handle_enum_done_intr(dev);
	 	if  (((intr_status & 0xffff) == 0x2420 ) ||((intr_status & 0xffff) == 0xa420)  ||((intr_status & 0xffff) == 0xa438)  ||((intr_status & 0xffff) == 0xa478))		
	 		{
			usb_connection_mode = USB_UNDEFINED;
	 		}
		goto	OK_OUT;

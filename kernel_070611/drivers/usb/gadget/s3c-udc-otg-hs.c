...
/*      //Henry:  unnecessary to do suspend/resume action
int usb_suspend_work_flag=0;
#ifdef CONFIG_STOP_MODE_SUPPORT
#include <linux/earlysuspend.h>
#endif
*/
int usb_connection_mode = USB_UNDEFINED;
struct usb_gadget_driver *global_driver;
void s3c_udc_cable_connect(struct s3c_udc *dev)
{
	...
	dev->is_pm_lock = true;
	usb_connection_mode = PC_LINK;
}

int udc_cable_status=STATUS_UDC_NOT_INSTALL;
void s3c_udc_cable_disconnect(struct s3c_udc *dev)
{
	...
	usb_connection_mode = USB_UNDEFINED;
}
...
//	writel(/*0x20*/0x2, S3C_USBOTG_PHYCLK);
...
//	adapter_usb_power_insert_flag = 10;
 ...
 */
extern void really_get_supplies(void);
static void udc_disable(struct s3c_udc *dev)
{
	...
	dev->gadget.speed = USB_SPEED_UNKNOWN;
	dev->usb_address = 0;
	//writel(readl(S3C_USBOTG_PHYPWR)|(0x7<<1), S3C_USBOTG_PHYPWR);		/* Henry Li: crash on EP1/EP3 */
	writel(readl(S3C_USBOTG_PHYPWR)|(1<<4), S3C_USBOTG_PHYPWR);

	dev->udc_state = USB_STATE_NOTATTACHED;
	udc_cable_status=STATUS_UDC_NOT_INSTALL;	/* Henry Li: rmmod g_file_storage */
	really_get_supplies();
	usb_connection_mode = USB_UNDEFINED;	
}
//---------------------------------------------------------------------------------------

void disable_udc_stop_mode()
{
	writel(readl(S3C_USBOTG_PHYPWR)|(0x1f), S3C_USBOTG_PHYPWR);
}
EXPORT_SYMBOL(disable_udc_stop_mode);
...
int usb_gadget_register_driver(struct usb_gadget_driver *driver)
{
	...
	retval = 0;
	g_reset_available = 1;
       while (udc_cable_status == STATUS_UDC_GET_SUPPLIES)
       	{
       	msleep(1);
		retval++;
		if (retval > 250)		// Henry: 5*50*20ms
			break;
	...
	retval = 0;
       while (udc_cable_status == STATUS_UDC_UNINSTALL)
       	...
       	msleep(1);
		retval++;
		if (retval > 50)		// Henry: 1*50*20ms
			break;
		if (gpio_get_value(S3C64XX_GPM(3)) == 0)
			return -EINVAL;
       	}
	udc_cable_status=STATUS_UDC_GADGET_DRIVER_INSTALL;	/* Henry Li: insmod g_file_storage */
#if 1
/*
...
if (!driver
	...
	else if (strcmp(driver->driver.name, "pocketbook") == 0)
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	int retval = 0;
       while (udc_cable_status == STATUS_UDC_RESET)
       	{
       	msleep(1);
		retval++;
		if (retval > 8000)
	...
	udc_cable_status = STATUS_UDC_UNINSTALL;
	if (!dev)
		return -ENODEV;
...
static void usb_up(struct work_struct *work)
{
...
static void s3c_udc_early_suspend(struct early_suspend *h)
{
  s3c_udc_power_up();
}

static void s3c_udc_late_resume(struct early_suspend *h)
{
   s3c_udc_power_down();
}

struct early_suspend s3c_udc_early_suspend_desc = {
        .level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
        .suspend = s3c_udc_early_suspend,
        .resume = s3c_udc_late_resume,
};
#endif
*/
/*
...
#ifdef CONFIG_STOP_MODE_SUPPORT
      register_early_suspend(&s3c_udc_early_suspend_desc);
#endif
*/	
	return retval;
}
...
#ifdef CONFIG_STOP_MODE_SUPPORT
        unregister_early_suspend(&s3c_udc_early_suspend_desc);
#endif
*/	
...
static enum	OTG_PM pm_policy = ALL_POWER_DOWN;//OPHYPWR_FORCE_SUSPEND;//ALL_POWER_DOWN;
...
//	struct s3c_udc *dev = platform_get_drvdata(pdev);
	...
	{
		case ALL_POWER_DOWN:			
			printk("%s:ALL_POWER_DONW\n",__FUNCTION__);
			//schedule_delayed_work(&usb_suspend_work,7*HZ);
			/* Henry Li@20101127 restore Samsung orignal statement for sleep mode flow */
			s3c_udc_power_up();
			break;
		case CLOCK_GATING:		
			printk("%s:CLOCK_GATE\n",__FUNCTION__);
...
static int __init udc_init(void)
{
	int ret;
	printk("_%s_",__FUNCTION__);
	INIT_DELAYED_WORK(&usb_suspend_work,usb_up);

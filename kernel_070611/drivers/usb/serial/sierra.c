...
#define DRIVER_VERSION "v.1.7.34"
...
static struct usb_serial *p_usb_serial = NULL;
#endif
...
/* Henry@2010-11-10 Add power management in sierra driver */
#ifdef CONFIG_STOP_MODE_SUPPORT
static int sierra_early_suspend(struct usb_serial *serial);
static int sierra_late_resume(struct usb_serial *serial);
struct early_suspend sierra_early_suspend_desc = {
        .level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
        .suspend = sierra_early_suspend,
        .resume = sierra_late_resume,
};
#endif
			...
			const struct usb_device_id *id)
{
	...
	if (p_usb_serial == NULL)
		{
		p_usb_serial = serial;
	       register_early_suspend(&sierra_early_suspend_desc);
		}
#endif
...
static const struct usb_device_id id_table [] = {
    ...
    { USB_DEVICE(0x1199,0x68A3) },/*sieera Wireless Aircard ls8082 */
	...
	if (tty) {
		...
		/* Otherwise the need to do non-composite mapping */
			...
			if (port->bulk_out_endpointAddress == 2)
				...
				interface = 2;
			else if (port->bulk_out_endpointAddress == 7)
			    interface=3;
			do_send = 1;
		}
	}
...
static int sierra_early_suspend(struct usb_serial *serial)
{
	pm_message_t useless;
	if (p_usb_serial != NULL)
		{
			sierra_suspend(p_usb_serial, useless);
		}
}
static int sierra_late_resume(struct usb_serial *serial)
{
...
static void sierra_shutdown(struct usb_serial *serial)
{
	...
	if (p_usb_serial != NULL)
	{
        unregister_early_suspend(&sierra_early_suspend_desc);
        p_usb_serial = NULL;
	}
#endif

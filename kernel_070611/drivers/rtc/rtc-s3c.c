...
void s3c_rtc_set_wakeupenable(int to)
{
	unsigned int tmp;

	pr_debug("%s: aie=%d\n", __func__, to);

	tmp = readb(s3c_rtc_base + S3C_RTCALM) & ~S3C_RTCALM_ALL;
	printk("tmp is %x\n",tmp);
	if (to)
		tmp |= S3C_RTCALM_ALL;
...
EXPORT_SYMBOL(s3c_rtc_set_wakeupenable);
...
//	printk(KERN_ALERT "%s cmd is 0x%08x arg is 0x%08x\n", __func__, cmd, arg);
	...
	case CM_DEEPSLEEP:
		if (arg)
		{
			printk("Switch to Sleep mode\n");
			power_set_mode(POWER_MODE_SLEEP);
		}
		else
		{
			printk("Switch to Stop mode\n");
		...
		case CM_ALARM_WAKEUP:
			...
			{
				printk("error arg for CM_ALARM_WAKEUP command\n");
			}
...
#ifdef CONFIG_STOP_MODE_SUPPORT
static struct platform_device *s3c_rtc_pdev = NULL;

static void s3c_rtc_early_suspend(struct early_suspend *h)
{
        if (s3c_rtc_pdev)
        {
                s3c_rtc_suspend(s3c_rtc_pdev, PMSG_SUSPEND);
        }
}

static void s3c_rtc_late_resume(struct early_suspend *h)
{
        if (s3c_rtc_pdev)
        {
                s3c_rtc_resume(s3c_rtc_pdev);
        }
}

struct early_suspend s3c_rtc_early_suspend_desc = {
        .level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
        .suspend = s3c_rtc_early_suspend,
        .resume = s3c_rtc_late_resume,
};
#endif
...
#ifdef CONFIG_STOP_MODE_SUPPORT
        s3c_rtc_pdev = NULL;
        unregister_early_suspend(&s3c_rtc_early_suspend_desc);
#endif
...
//#ifdef CONFIG_STOP_MODE_SUPPORT
//        s3c_rtc_pdev = pdev;
//        register_early_suspend(&s3c_rtc_early_suspend_desc);
//#endif

...
static int s3c_i2c_set_master(struct s3c_i2c *i2c)
{
	...
	while (timeout-- > 0) {
		...
		udelay(300);//200 300
		//binchong: improve the boot time 
	}
//退出while表示一直在寫，讀不到
	iicstat = readl(i2c->regs + S3C_IICSTAT);
	iicstat &= ~S3C_IICSTAT_BUSBUSY; //強制使寫終止，使能開始讀
	writel(iicstat, i2c->regs + S3C_IICSTAT);
	#if 0
...
static int s3c_i2c_init(struct s3c_i2c *i2c)
{
	...
	dev_info(i2c->dev, "S3C_IICCON=0x%02lx\n", iicon);
...
{
	struct s3c_i2c *i2c = platform_get_drvdata(dev);
// fix resume i2c clock issue +
	writel(0x7, i2c->regs + S3C_IICCON);
	writel(0, i2c->regs + S3C_IICSTAT);
// fix resume i2c clock issue -	
	i2c->suspended = 0;
	s3c_i2c_init(i2c);
...
//&*&*&*HC1_20100129, Modify for DVFS operation
//module_init(i2c_adap_s3c_init);
subsys_initcall(i2c_adap_s3c_init);
//&*&*&*HC2_20100129, Modify for DVFS operation

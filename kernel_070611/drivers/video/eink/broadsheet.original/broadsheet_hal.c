			...
			counter = 0;			
				for(i=left;i<left_width;i+=2) 
				{
					int addr = (j)*(XRES/ratio)+i/ratio;
						colorsample_pixel[counter] = bs_4bpp_nybble_swap_table_inverted[epd_base[addr++] ];
						counter++;
						if ( counter == 2 )	
							{
							counter=0;
...
static bool broadsheet_ep3_show_display_image(int para_update_mode)
{
	...
	if(update_mode == 0x20/*fx_update_full*/)
	{
		value = 0x4200 | (get_recycle_lut_number() << 4);
		epd_send_command(0x33,value);	
	}
	else
	{	
		...
		epd_send_command(0x35,value);
#else	
		value = (0x42f0);		//Henry: Check 0x4200 latter ??? Display Update LUT Select bits 3-0 = 0???
		epd_send_command(0x35,value);
#endif		
	}

...
static unsigned char einkfb_dynamic 	=0;
...
static int global_suspend_resume_state=1; 
 ...
 *
 */
static int eink_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	return 0;
}

/*
 *      eink_set_par - Optional function. Alters the hardware state.
 *      @info: frame buffer structure that represents a single frame buffer
 *
 */
static int eink_set_par(struct fb_info *info)
{
	return 0;
}
 ...
 */
static int eink_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{		
	
    //hal_ops.hal_pan_display(var, info);		
    //printk("eink_pan_display\n");		
    
	return 0;
}
static int eink_setcolreg(unsigned int regno, unsigned int red, unsigned int green, unsigned int blue, unsigned int transp, struct fb_info *info)
{
	return 0;
}
...
/* Henry Li: preserve bpp setting*/
void einkfb_set_bpp(unsigned long bpp)
{
	if ((bpp == EINKFB_1BPP) || (bpp == EINKFB_2BPP) || (bpp == EINKFB_4BPP))
		einkfb_bpp = bpp;
}
EXPORT_SYMBOL(einkfb_set_bpp);
...
void einkfb_set_dynamic_parameter(unsigned char parameter)
{
	if ((parameter == 0) || (parameter == 1))
		einkfb_dynamic = parameter;
}
EXPORT_SYMBOL(einkfb_set_dynamic_parameter);

unsigned char einkfb_get_dynamic_parameter(void)
{
	return einkfb_dynamic;
}
EXPORT_SYMBOL(einkfb_get_dynamic_parameter);
...
static void BLANK_INIT(void)
{
	...
	//
//&*&*&*JJ1_20091213 for update performance	
...
//&*&*&*JJ2_20091213 for update performance	
...
//&*&*&*20091210ADD1 EPD bootup LOGO
extern void epd_show_logo(unsigned short cmd);
//extern void epd_show_second_logo(unsigned short cmd);
//&*&*&*20091210ADD2 EPD bootup LOGO
...
extern bool quick_resume_hook(void);		//Henry: For PB quick resume 0927
extern void broadsheet_check_finished(void);
extern void einkfb_block(void);
extern void einkfb_unblock(void);

void broadsheet_suspend_early(void)
{
/*
	einkfb_block();
	broadsheet_check_finished();
	//EINKFB_BLANK(FB_BLANK_HSYNC_SUSPEND);
	einkfb_unblock();	
*/	
}
EXPORT_SYMBOL_GPL(broadsheet_suspend_early);
extern bool bs_quickly_poweroff(void);		//temporary
void einkfb_set_global_suspend_resume_state(int value)
{
	global_suspend_resume_state=value;	
}
int  einkfb_get_global_suspend_resume_state(void)
{
	return global_suspend_resume_state;	
}
EXPORT_SYMBOL_GPL(einkfb_set_global_suspend_resume_state);
EXPORT_SYMBOL_GPL(einkfb_get_global_suspend_resume_state);

extern unsigned char get_EPD_type(void);
extern void set_EPD_type(unsigned char value);
static int einkfb_x3_suspend_resume_hook(bool which)
{
	int result = EINKFB_FAILURE;
	...
	switch ( which )
	{
		case EINKFB_SUSPEND:
			...
			if (gpa3 == 0x01){
				...
				{
					printk("xuyuda: sc8: %s\n", __FUNCTION__);
					msleep(800);		//Henry: 2010-12-09 SC8 EPD
				}
			}
#endif
			result = false;
			bs_quickly_poweroff();			// 1018 for testing only
		break;
		
		case EINKFB_RESUME:
			einkfb_block();			
			//result = EINKFB_BLANK(FB_BLANK_UNBLANK);
			result = quick_resume_hook();	//Henry: For PB quick resume 0927
			einkfb_unblock();	
			global_suspend_resume_state=1;
			result = false;			
		break;
	}
...
extern bool bs_hw_test(void);
extern bool bs_wr_one_ready(void);

static struct platform_device *einkfb_pdev = NULL;

static void einkfb_early_suspend(struct early_suspend *h)
{
#if defined(CONFIG_HW_EP3_DVT) || defined(CONFIG_HW_EP4_DVT) 
#if 0
//        if (einkfb_pdev)
//        {
//                einkfb_suspend(einkfb_pdev, PMSG_SUSPEND);
//        }
//	bs_cmd_stby();
	global_suspend_resume_state=0;			
	//bs_cmd_run_sys();
...
//	mdelay(25);
#else
	einkfb_x3_suspend_resume_hook(EINKFB_SUSPEND);
#endif		//End of defined(CONFIG_HW_EP3_DVT) || defined(CONFIG_HW_EP4_DVT) 
...
static void einkfb_late_resume(struct early_suspend *h)
{
#if defined(CONFIG_HW_EP3_DVT) || defined(CONFIG_HW_EP4_DVT) 
#if 0
//        if (einkfb_pdev)
//        {
//                einkfb_resume(einkfb_pdev);
//        }
//	BS_SEND_CMD(0x02);
//	mdelay(10);
	bs_wr_one_ready();
	bs_cmd_run_sys();
	printk("bs_wr_one_ready:%d\n", bs_wr_one_ready());
	global_suspend_resume_state=1;
//	mdelay(200);
//	printk("bs_hw_test() - result = %d\n", bs_hw_test());
#else
	einkfb_x3_suspend_resume_hook(EINKFB_RESUME);
#endif		//End of defined(CONFIG_HW_EP3_DVT) || defined(CONFIG_HW_EP4_DVT) 
#elif  defined(CONFIG_HW_EP1_DVT)  || defined(CONFIG_HW_EP2_DVT)
	unsigned long gpdata, gpa3;
    gpdata = get_EPD_type();
...
struct early_suspend einkfb_early_suspend_desc = {
        .level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
        .suspend = einkfb_early_suspend,
        .resume = einkfb_late_resume,
};
#endif
...
//&*&*&*HC1_20091208, add for suspend/resume function
	einkfb_set_suspend_resume_hook(einkfb_x3_suspend_resume_hook);
//&*&*&*HC2_20091208, add for suspend/resume function

#ifdef CONFIG_STOP_MODE_SUPPORT
        einkfb_pdev = dev;
        register_early_suspend(&einkfb_early_suspend_desc);
#endif
...
#ifdef CONFIG_STOP_MODE_SUPPORT
        einkfb_pdev = NULL;
        unregister_early_suspend(&einkfb_early_suspend_desc);
#endif
	...
	if ( !ret )
	{
		...
		if ( einkfb_device )
		{
			...
			if ( !ret )
			{
				...
				if ( !ret )
				{
//&*&*&*HC1_20091231, remove the notification for power off issue					
					//register_reboot_notifier(&einkfb_reboot_nb);
//&*&*&*HC2_20091231, remove the notification for power off issue						

...
#define EINK_4BPP               4
#define EINK_8BPP               8
#define EINK_16BPP				16
#define EINK_BPP_MAX			EINK_16BPP
...
//
#define UPDATE_PART(f)        (fx_update_partial == (f))

#define UPDATE_FULL(f)        (fx_update_full == (f))
...
struct update_area_u16
{
    ...
    fx_type	    which_fx;                   // FX to use.
    int					frameno;										//buffer index number
    int					cleanbk;										//1=clean background to white
    int					reserve[3];									        
    __u16       *buffer;                    // If NULL, extract from framebuffer, top-left to bottom-right, by rowbytes.
};
typedef struct update_area_u16 update_area_u16;
 ...
 */
#if 1
#define FBIO_EINK_UPDATE_DISPLAY            _IO(FBIO_MAGIC_NUMBER, 0xdb) // 0x46db (fx_type: fx_update_full || fx_update_partia)
#define FBIO_EINK_UPDATE_DISPLAY_AREA       _IO(FBIO_MAGIC_NUMBER, 0xdd) // 0x46dd (update_area_t *)
#define FBIO_EINK_PRELOAD_TO_BUFFER	        _IO(FBIO_MAGIC_NUMBER, 0xe0) // 0x46e0 
#define FBIO_EINK_PRELOAD_TO_UPDATE		      _IO(FBIO_MAGIC_NUMBER, 0xe1) // 0x46e1 
#define FBIO_EINK_SET_BOOTUP_LOGO						_IO(FBIO_MAGIC_NUMBER, 0xe2) // 0x46e2
#define FBIO_EINK_HANDWRITE_FLAG						_IO(FBIO_MAGIC_NUMBER, 0xe3) // 0x46e3
#else
#define	FBIO_EINK_UPDATE_DISPLAY	    _IO(FBIO_MAGIC_NUMBER, 0x33) // 0x4633
#define FBIO_EINK_UPDATE_DISPLAY_AREA	    _IO(FBIO_MAGIC_NUMBER, 0x34) // 0x4634
#endif




#define FBIO_EINK_RESTORE_DISPLAY           _IO(FBIO_MAGIC_NUMBER, 0xef) // 0x46ef (fx_type: fx_update_full || fx_update_partia)
...
/**********************************
 *new ioctl interface for x3
 *zhao
 */
#if 0
#define	X3_UPD_FULL		_IO(FBIO_MAGIC_NUMBER,0x33)
#define	X3_UPD_FULL_AREA	_IO(FBIO_MAGIC_NUMBER,0x34)
#define	X3_UPD_PART		_IO(FBIO_MAGIC_NUMBER,0x35)
#define	X3_UPD_PART_AREA	_IO(FBIO_MAGIC_NUMBER,0x36)

#define	X3_S1D13521_UPD_FULL	_IO(FBIO_MAGIC_NUMBER,0x4500|UPD_FULL)
#define	X3_S1D13521_UPD_FULL	_IO(FBIO_MAGIC_NUMBER,0x4500
#endif

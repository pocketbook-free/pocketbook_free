...
#include <plat/cpu.h>
#include <plat/pll.h>
#include <plat/sdhci.h>			//William add
...
struct clksrc_clk clk_mout_epll = {
	.clk	= {
		.name		= MOUT_EPLL_CLK, //"mout_epll",		//William modify
		.id		= -1,
...
void __init_or_cpufreq s3c64xx_setup_clocks(void)
{
	...
	printk(KERN_DEBUG "%s: xtal is %ld\n", __func__, xtal);
		__raw_writel(0, S3C_EPLL_CON1);
		__raw_writel(0, S3C_EPLL_CON0);
		__raw_writel((1<<31)|(132<<16)|(3<<8)|(0<<0) ,S3C_EPLL_CON0);  //528M
	...
	clk_p.rate = pclk;
	clk_f.rate = fclk;
	clk_epll.rate = epll;			//William add 20100918
...
//	clk_set_rate(&clk_mmc2.clk, 50*MHZ);
	//printk("S3C_EPLL_CON0 0x%08x \n", __raw_readl(S3C_EPLL_CON0));
	//printk("S3C_EPLL_CON1 0x%08x \n", __raw_readl(S3C_EPLL_CON1));
	//printk("S3C_CLK_SRC 0x%08x \n", __raw_readl(S3C_CLK_SRC));
	//printk("S3C_CLK_SRC2 0x%08x \n", __raw_readl(S3C_CLK_SRC2));
	//printk("S3C_OTHERS 0x%08x \n", __raw_readl(S3C_OTHERS));
}
